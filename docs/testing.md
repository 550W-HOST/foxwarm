...
The full-application harness starts the real compiled server and built WebUI against a generated data root, synthetic token, seeded authority files, and loopback ephemeral ports. A local scripted server implements OpenAI Responses SSE and Chat Completions SSE; only the provider network boundary is mocked. Session routing, tools, persistence, authentication, realtime delivery, and browser rendering remain production paths. Unexpected provider requests fail, held streams remain bounded by scenario and harness teardown, and failures retain application/provider/test logs plus a diagnostic screenshot and browser console.
