...
One readiness interpretation governs discovery, create, ensure, inspect, lifecycle details, and capability execution. A container is execution-ready only when Docker reports exact `running` state with no paused, restarting, or dead flag. Descriptor availability and lifecycle `details.availability` use that same result, while `details.status` reports the exact normalized Docker status. Create inspects the new container before committing provider state and removes that exact container if it exited or otherwise failed readiness. Ensure preserves container ID, generation, and artifacts: it starts an exact created/exited/stopped container or unpauses an exact paused container, then re-inspects before returning ready. Restarting, dead, inconsistent, unknown, and unavailable states remain non-ready and are never presented as usable; inspect reports unavailable provider state explicitly rather than fabricating a Docker status.
...
The provider advertises a read-write filesystem backend and exec. Foxwarm Core derives canonical `read`, `write`, `edit`, and `apply_patch`; the container helper knows only parent/stat/ranged-read/readdir/whole-write/mkdir/remove and never parses edit or patch grammar. File mutation remains worktree-only. Internal Session `read` may also open exact retained execution logs/artifacts by their canonical absolute paths; external MCP callers cannot use file tools to read that artifact directory and must query their own live-context commands through `foxwarm_exec_result`. Shared inline-image results are preserved. Node write schemas accept literal content rather than Main-local `contentRef` cache handles. Browser, PTY, Code, fixed-service, and copy capabilities remain absent.

`exec` reuses the shared persistent manager and output formatter. The manager writes scripts, registry, logs, status, and cwd files only in the exact generation artifact directory, then launches its generated Bash script through the configured fixed Docker launcher with `shell:false`. The container receives only `TERM` and the manager's `FOXWARM_EXEC_*` values; Main environment, Session authority, Docker credentials, and the Docker socket are not passed into it. Foreground output, timeout clamping, timeout-to-background notices, bounded excerpts, exit reporting, cwd notices, and retrying background completion use canonical Foxwarm behavior. For internal Session commands, completion enters the exact source Session through Main's authoritative system-event path, including startup reconciliation after Main restarts. An external MCP command uses an exact generated owner, a separate generation-local artifact directory, and a Main-local completion callback with no Session event. The original live MCP context alone can retrieve bounded results; closing it does not kill an already-started command. A destroyed and recreated Node ID never redirects a running query to the new generation.
