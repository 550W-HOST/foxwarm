...
The provider advertises a read-write filesystem backend and exec. Foxwarm Core derives canonical `read`, `write`, `edit`, and `apply_patch`; the container helper knows only parent/stat/ranged-read/readdir/whole-write/mkdir/remove and never parses edit or patch grammar. File mutation remains worktree-only; `read` may also open exact retained execution logs/artifacts by their canonical absolute paths. Shared inline-image results are preserved. Node write schemas accept literal content rather than Main-local `contentRef` cache handles. Browser, PTY, Code, fixed-service, and copy capabilities remain absent.
