...
- Heavy/non-direct system-like messages render as compact left-side thread cards sharing the ToolTag/ThreadLineButton structure used by tool and Reasoning cards. Their tag comes from `getSystemMessagePreviewDescriptor`: heavy `<foxwarm-system kind="...">` first, otherwise non-channel `<foxwarm-message type="...">`, otherwise `system`. Collapsed previews can prefix inter-agent `sourceSessionId`, session-boundary `event`, or event `type`; when an inter-agent source is present its collapsed prefix is a SessionHashLink that does not expand the card. Expanded body text remains raw/readable but links only the `sourceSessionId` value in canonical inter-agent opening tags. The chrome uses the shared yellow palette and mapped Lucide icons: session-boundary uses SeparatorHorizontal, reminders/prompt/lifecycle kinds use their own stable semantic icons, and unknown system kinds use Bell (whereas unknown real tools use Wrench). Preview/body text uses the ordinary slate foreground from tool/thread content. Cards retain the same local `my-0.5` rhythm as other thread cards; `MessageRow` groups consecutive model/tool/heavy-system rows without its ordinary top-level separation, but direct user rows still begin/end a turn. Inline direct-user text still renders tag lines as small metadata while body lines remain normal text. The classifier distinguishes real direct channel wrappers (`type="channel"`) and lightweight time/session metadata from heavy/non-direct wrappers (`type="inter-agent"`, `timer`, `trigger`, etc.), pure system event tags such as wait timeouts, and heavy snapshot/system-prompt tags, so only the latter group goes left through `SystemLikeMessageCard`.
