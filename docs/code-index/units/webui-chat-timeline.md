...
- [2026-07-24] The usage badge is the canonical interaction boundary for token detail disclosure. Preserve its existing collapsed compact labels, placement, setting gate, and aggregate totals; on mouse/keyboard activation only the badge expands in place to full labels plus persisted time and routing attribution. A virtual route is displayed as `virtual → concrete`; route/time metadata is never inferred from mutable UI state. For grouped calls, retain aggregation but show all unique routes and a time range rather than assigning the aggregate to one call.
