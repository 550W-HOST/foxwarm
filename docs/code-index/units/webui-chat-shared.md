...
- `renderMarkdownSegments` / `renderMarkdownSegmentsWithSanitizer` — render the same complete lexer result as stable top-level ordinary HTML segments
...
- `MarkdownHtmlSegment` — memoized sanitized-HTML leaf whose stable token key lets unchanged streamed blocks retain DOM identity
...
- `SessionStreamEvent` mirrors both legacy cumulative stream fields and version-2 offset delta/tool-argument fields.
...
- Ordinary fenced code in `.foxwarm-markdown` uses a compact `0.5rem` vertical margin while retaining wrapped, contained overflow behavior. Special-block descendants remain excluded from this ordinary prose rule.
...
- `summarizeToolTagCounts` folds a tag-item list into one item per distinct visible label, relabeled `<label> ×N`, so repeated tool calls collapse instead of repeating identical tags. Errored items (`error` tone) are counted in their own entry per label, so a mixed group shows the successful total and the failed total separately instead of one failed entry. Ordering is count descending with first-appearance tiebreak. The error entry keeps the `error` tone; in the non-error entry the error tone cannot occur, success upgrades a neutral entry, and every other tone stays as first seen. Each folded item keeps its representative `name`, so icons and the `data-tool-tag-tone` hook are unchanged for the folded tag.
