...
- `renderSystemTextWithSessionLinks` recognizes `sessionId: \`...\``, `session \`...\``, and inter-session tool output like `Child session created: \`...\`` so model/system text and tool renderers share link behavior.
...
- `toolMeta`/`ToolTag` maps tool-like names to icons (lucide-react), colors, and display labels for consistent UI rendering; non-tool timeline cards such as `ctx-block` can reuse the same tag primitive.
