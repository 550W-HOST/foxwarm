...
- **Worker placement fences**: Current-session effects carry a trusted internal local/Session-worker marker into direct and nested ToolContexts. Reachable operations that still need unclosed Main topology, managed ToolScript, compaction, destructive lifecycle, or bootstrap/pairing fail with retryable `SESSION_WORKER_TOOL_UNAVAILABLE` before tool-start observability, concrete permissions, or handler initialization. Guard normalization mirrors handler action/target semantics, including literal-only `remote_node`/`node_tools` action matching. Exact current status/display/stop/archive/settings, same-agent vector recall, node list/select/search, compound copy, Main-owned file delivery, remote image handoff, and master-local safe operations remain enabled through exact owner/fixed facades. Child creation, the catalog session list, and cross-session message reads route through the fixed main-management facade (detached authority reads for fenced targets); cross-session control/settings, `stop_session` beyond the current session, and unscoped `get_memory_context` remain fenced. Worker selection never performs a stale catch-time rollback after the authoritative persistence hook may have resynced/poisoned the hot owner.
