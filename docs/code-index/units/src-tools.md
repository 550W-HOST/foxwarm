...
- **Atomic send completion**: `send_to_session` and `create_child_session` expose one `afterSend` enum: continue, finish idle without wait state, or wait for later activity expected from the resolved target. Completed child reports use `finish`; only genuine reply dependencies use `wait`. Hidden legacy booleans remain runtime-readable but are absent from the model schema. Canonical contract: [D-pipeline-handoff-wait](../threads/message-processing-pipeline.md#d-pipeline-handoff-wait).
