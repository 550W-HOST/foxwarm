...
| `setDefinitionsRef` | Wires definitions and direct-exposure metadata used by discovery |

### tools/resolvedTools.ts — Canonical invocation resolution
| Function | Description |
|----------|-------------|
| `resolveDirectTool` | Resolves direct provider and retained compatibility invocations into one builtin/node/MCP operation |
...
| `executeResolvedTool` | Applies existing placement/permission behavior and dispatches through local, Node, or MCP owners |
