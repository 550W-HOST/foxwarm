...
- **MCP configuration disclosure**: `mcp_config` and `list_mcp_servers` remain registered hidden builtins invoked through `call_tool`; the bundled `mcp-management` skill provides setup guidance without injecting their schemas into ordinary provider requests.
...
### D-tools-mcp-live-configuration

[2026-08-01] MCP configuration changes must go through the `mcp_config` handler so persisted data and the runtime configuration source remain coherent. Successful changes are visible to subsequent MCP listing, discovery, and calls immediately and require no Foxwarm restart. Agents must not manually edit the state MCP configuration file because manual edits do not update the live cached configuration immediately. Safe listing uses `list_mcp_servers`, and both management builtins are discovered/invoked through `search_tools`/`call_tool` rather than injected by default.
