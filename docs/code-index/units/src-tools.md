...
- **Worker placement fences**: Current-session effects carry a trusted internal local/Session-worker marker into direct and nested ToolContexts. Reachable operations that still need unclosed Main topology, cross-session coordination, managed ToolScript, compaction, destructive lifecycle, bootstrap/pairing, delivery, or remote-image state fail with retryable `SESSION_WORKER_TOOL_UNAVAILABLE` before tool-start observability, concrete permissions, or handler initialization. Guard normalization mirrors handler action/target semantics. Exact current status/display/stop/archive/settings, same-agent vector recall, node list/select/search, compound copy, and master-local safe operations remain enabled through exact owner/fixed facades. Unscoped `get_memory_context` remains fenced.
