...
- **Worker placement fences**: Current-session effects carry a trusted internal local/Session-worker marker into direct and nested ToolContexts. Reachable operations that still need Main topology, cross-session coordination, managed ToolScript, compaction, or destructive lifecycle fail with retryable `SESSION_WORKER_TOOL_UNAVAILABLE` before tool-start observability, concrete permissions, or handler initialization. Guard normalization mirrors each handler's trimmed action and literal-vs-fallback target semantics; aliases remain current. Exact current status/display/stop/archive/settings, same-agent current-agent vector recall, and master-local safe operations remain enabled. Unscoped `get_memory_context` is fenced until it has a source-scoped contract.
