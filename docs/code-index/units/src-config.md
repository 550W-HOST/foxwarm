...
- `mcpInbound` is a strict startup-only `{ enabled, identities }` block, disabled when absent. Malformed disabled blocks still fail validation; enabled configurations need independently authenticated external IDs. Enabling it starts `/mcp` on the existing HTTP listener, with no Foxwarm tools currently connected. See [D-config-mcp-inbound-foundation](./src-mcp-inbound-config.md#d-config-mcp-inbound-foundation) and [D-mcp-inbound-http-transport](./src-mcp-inbound-http.md#d-mcp-inbound-http-transport).
