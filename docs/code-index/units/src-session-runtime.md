...
- **Controls:** stop uses the closed supervisor/Worker interrupt path. Retry ensures the exact Worker owner and runs the canonical retry turn (`parts:null`) directly inside that owner, with an optional serialized QueueSource registered against the live Main channel context for ordinary final-delivery parity; it creates no second queue/mailbox intent and rejects validation/unknown/busy admission definitely. For the current call, only transport unavailable/send-failed/closed loss is normalized to `SESSION_WORKER_RETRY_OUTCOME_UNKNOWN`: the retry may already be committed/delivered, so callers warn and require a history check rather than claiming failure or retrying automatically. Serialized handler `RPC_CANCELLED` and `RPC_DEADLINE_EXCEEDED` remain definite; the retry call does not currently supply a caller signal or deadline. Dequeue remains unsupported for worker-fenced sessions. Local placement retains existing semantics.
...
- Worker-owned history reads one atomic per-session JSON file directly after that exact-owner load, replaces a detached read-only owner with the current state format, renders/annotates its frontier, and externalizes the returned snapshot. Missing, malformed, or unsupported authority fails retryably without backup recovery or catalog fallback; legacy upgrade remains Worker-owned.
