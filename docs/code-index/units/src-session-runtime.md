...
- **Controls:** stop uses the closed supervisor/Worker interrupt path when an exact Worker owns the session; dequeue and retry remain unsupported for worker-fenced sessions and fail retryably instead of mutating Main-local semantic state. Local placement retains existing session-manager/router semantics.
