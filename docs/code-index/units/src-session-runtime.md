...
- Worker ingress accepts only an exact canonical ID and current ordinary QueueItem, preserves its normalized message/source/blob-reference/image/client identities in one mailbox intent, requires one already-ready durable/live generation before append, and calls only that generation's existing `runPending`. A fixed exact-key plain-data validator runs before registry/coordinator/store use and caps the normalized UTF-8 item at 1 MiB; malformed, accessor, cyclic, nonfinite, unknown, and overbound input never reaches the mailbox. Post-append failure stays durable/ambiguous. Its source-context registry is Main-memory-only, full-QueueSource-keyed, ambiguity rejecting, and cleaned in `finally`.
