...
Queue-origin canonical append presentation remains transient and separate from the committed projection. The Worker forwards one ordered batch after persistence, with a bounded sanitized hot-queue preview and its exact mailbox cursor. Main composes later durable mailbox inputs after that cursor before WebUI fan-out, so the batch cannot hide a newer pending tail. Ordinary model messages retain the model-stream flush barrier and individual presentation path.
