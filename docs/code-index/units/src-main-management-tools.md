...
- `mainManagementToolServiceDescriptor` — version 3 descriptor with closed `execute` and separate internal `scheduleWaitTimeout` methods.
- `MAIN_MANAGEMENT_TOOL_OPERATIONS` — exact 19-operation allowlist: messaging, agent listing, timer CRUD, child creation, session catalog/display/message reads, archive/recall reads, agent/session creation, and node bootstrap/pairing.
