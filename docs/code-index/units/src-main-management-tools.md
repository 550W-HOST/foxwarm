...
The handler rejects missing/deleted source sessions before dispatch, caps serialized args at 64 KiB, and repeats generic builtin authorization at the Main-owned boundary. Operations needing source settings or permissions receive one detached read-only authority loaded inside Main plus a process-local read marker; vector scope resolution trusts that detached source without hydrating or persisting the catalog stub. Other operations reconstruct only `{ sessionId }`, except `send_to_session`, which also receives one internal result-metadata request so a successful reverse call can return its exact resolved target to the Worker turn owner. Isolation, relation, channel, target-session, and timer-scope checks remain in the existing raw handlers. `set_tool_rules` receives the detached exact source, repeats current-policy authorization for itself and `node:master/read` of the candidate, and atomically installs validated exact bytes.

The `create_child_session` operation accepts only bounded cloneable args (suffix, fork, message, node, forceModel, afterSend, plus hidden legacy stop/wait controls), while `create_session` has its own closed creation-key set. Its `node` field is a semantic child-placement argument, not a selector for where the Main-owned management tool executes: canonical resolution preserves it for generic authorization, the service repeats that authorization on Main with the same field, and the effect commits an accepted explicit Node or inherits on omission. Main-management and the local raw handlers call the same strict non-mutating creation normalizers before effects: they reject removed or unknown top-level keys, conflicting new/legacy send controls, and malformed/unknown nested `forceModel: { modelId?, effort? }` content, clone the accepted nested object, and preserve configured-model/canonical-effort validation. Every child creation from a worker-fenced source derives current inherited settings—and fork history when requested—from the authoritative JSON through a strictly read-only detached read supplied to the canonical creation path as a never-persisted source override; Main never hydrates or writes the fenced parent authority. `create_agent` and `create_session` reuse that detached source for inherited settings while leaving it unchanged; source conversion and explicit another-source agent creation remain fenced. `delete_session` accepts exactly one bounded target ID, preserves the model isolation check, and invokes the Main-owned deletion orchestrator with repeated exact source-generation assertions; canonical source/alias self-delete fails before target preparation. `session_list` renders the Main-owned catalog; `session_update_display_name` performs the Main-owned catalog/presentation rename; `get_session_messages` uses the exact-owner detached SessionRuntime read. Cross-session `recall` and hidden archive readers execute exact source-range reloads in Main, preserve existing isolation/agent checks and preview bounds, and use the selected vector facade. Node bootstrap/list/approval operate only on Main-owned topology/registry state.
