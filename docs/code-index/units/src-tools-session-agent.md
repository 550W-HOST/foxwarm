...
| `renderContextPreviewItems` | Applies staged literal/regex post-filters, optionally caps the filtered item set with a truthful selection notice, returns per-stage exclusion counts, renders filter notices/match-centered snippets, and enforces total-budget truncation |
