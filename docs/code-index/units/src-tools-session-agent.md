...
- `send_to_session(waitForReply:true)` emits a hidden post-batch generic-wait request only after delivery succeeds. `create_child_session(waitForReply:true)` requires a non-empty initial message and awaits its delivery before emitting the same request; ordinary unflagged child creation retains its existing asynchronous initial-send behavior. Canonical orchestration: [D-pipeline-handoff-wait](../threads/message-processing-pipeline.md#d-pipeline-handoff-wait).
