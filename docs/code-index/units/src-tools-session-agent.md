...
- These tool functions are authoritative raw handlers. Most are invoked directly by the builtin dispatcher; `send_to_session`, `send_to_channel`, `list_agents`, timer CRUD, Worker session list/display-name, child creation, and Worker cross-session message reads use the closed Main Management RPC service where Main owns the operation.
