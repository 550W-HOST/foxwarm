...
Files: website/package.json, website/package-lock.json, website/wrangler.jsonc, website/astro.config.mjs, website/tsconfig.json, website/.gitignore, website/scripts/sync-assets.mjs, website/tests/site.test.mjs, website/public/robots.txt, website/src/content.config.ts, website/src/pages/index.astro, website/src/pages/404.astro, website/src/styles/home.css, website/src/styles/docs.css, website/src/assets/foxwarm-wordmark-light.svg, website/src/assets/foxwarm-wordmark-dark.svg, website/src/content/docs/docs/index.mdx, website/src/content/docs/docs/installing.md, website/src/content/docs/docs/model-setup.md, website/src/content/docs/docs/model-options.md, website/src/content/docs/docs/history-search.md, website/src/content/docs/docs/mcp-inbound.md, website/src/content/docs/docs/agents-sessions-memory.md, website/src/content/docs/docs/tools-skills-mcp.md, website/src/content/docs/docs/nodes.md, website/src/content/docs/docs/channels.md, website/src/content/docs/docs/data-upgrades-backups.md, website/src/content/docs/docs/faq.md, .github/workflows/website.yml
...
- `src/content/docs/docs/` owns user guides for installation, model setup/options, Agents and Sessions, compaction/history search, tools/Skills/MCP, inbound MCP, Nodes, Channels, backups/upgrades, and FAQ. The `/docs/` article and Starlight sidebar link these guides; detailed implementation references remain in the repository.
...
- `wrangler.jsonc` provides a separate Cloudflare Workers Static Assets preview target named `foxwarm-preview`, serving only `dist/` with the custom 404 page and version preview URLs. Workers Builds uses `website` as its root and `npm ci && npm run check` as its build command. Production-branch deployment uses `npx wrangler deploy`; non-production branches use `npx wrangler versions upload`. This target does not replace the canonical GitHub Pages deployment or configure its domain.
...
README provides project orientation, a short installer path, concepts, and documentation entry points. User setup examples belong in the website guides; the Channels guide owns its starter configuration rather than linking back to README. README installation commands and documentation links use the canonical public domain. The PowerShell installer's header example uses the same URL, but installer behavior remains unchanged. Canonical architecture and deployment ownership are recorded in [D-website-independent-static-site](../modules/website.md#d-website-independent-static-site).
