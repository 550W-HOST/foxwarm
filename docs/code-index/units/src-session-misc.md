...
- **Archive**: Canonicalizes image parts through the shared blob store before committing SQLite rows. New archive rows contain blob references rather than inline base64 or per-session filesystem paths; compatible readers retain old path records. Creation rollback can remove a partial append for a known uncommitted lifetime.
