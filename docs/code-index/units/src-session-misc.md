...
- `resolvePermittedSessionTarget`, `sendToSession` — inter-session messaging with special target resolution (`<main>`, `<parent>`), isolation checks, self-send rejection, and structured source metadata
...
| `sendToSession(deps, targetSessionId, message, fromSessionId)` | ~152 | Sends an inter-session message as one `<foxwarm-message type="inter-agent" ...>body</foxwarm-message>` system part; rejects self-sends and returns requested/resolved IDs |
...
- **Archive**: Extracts inline binary data (images) from message parts, writes them to disk with content hashing, replaces inline data with reference metadata, appends JSONL records to archive log and store.
