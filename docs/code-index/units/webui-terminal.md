...
| `TerminalVirtualKeyboard(props)` | TerminalVirtualKeyboard.tsx | Renders fixed special keys, stable-height ABC/123/More pages, and mode footer |
...
- Web mode makes xterm's textarea read-only with `inputmode=none` so tapping the terminal does not summon the mobile IME or native form accessory bar. Native mode restores the textarea and focuses it synchronously from the mode button. Collapsed mode keeps a minimal mode footer; on fine-pointer desktops it leaves the textarea's normal input behavior intact.
- The fixed special bar contains Esc, Tab, one-shot Ctrl/Alt, arrows, and More. ABC and 123 pages use four familiar phone-keyboard rows; Shift is one-shot or double-tap locked and changes visible labels. More replaces the body at the same height with Home/End/PgUp/PgDn, Insert/Delete, Copy/Paste, and F1-F12, then returns to its source page.
...
- The keyboard is a flex child of the terminal pane, so its fixed body/footer and safe-area padding naturally participate in ResizeObserver/FitAddon refits. Native mode compares the keyboard container's stable, untransformed bottom with the `visualViewport` visible bottom and moves the footer only by their local overlap; visual-viewport, window, and container resizes recompute it so split panes do not inherit a global full-keyboard translation.
