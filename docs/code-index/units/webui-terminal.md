...
Files: packages/webui/src/components/TerminalView.tsx
...
| `requestedCwd` (useMemo) | ~52–57 | Normalizes the `initialCwd` prop into a trimmed string or undefined |
...
- On mount, resolves a terminal ID by: checking an explicit `initialTerminalId`, listing existing terminals for cwd-based reuse, or creating a new one via POST with `cwd`, `nodeId`, `cols`, and `rows`.
...
- Designed to be embedded in the WebUI workbench shell; chat/session context may provide an initial cwd, but backend terminal creation no longer receives or stores a session id.
