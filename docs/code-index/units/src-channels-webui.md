...
- Archive accepts optional `includeDescendants` only for archive-to-true and returns matched/changed IDs and counts. Delete accepts optional `includeDescendants`, recomputes the canonical subtree, preflights every selected session for channel/busy blockers, deletes deepest-first, reports partial progress on unexpected failures, and detaches direct survivors for single-session deletion. Canonical semantics: [D-lifecycle-descendant-actions](../threads/session-lifecycle.md#d-lifecycle-descendant-actions).
