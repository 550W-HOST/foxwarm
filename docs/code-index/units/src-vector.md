...
[2026-08-29] Exact-session hybrid lookup may consume safe partial persistent FTS hits while coverage is incomplete or startup backfill is active; only readiness outside rebuild/shutdown is required to query the derivative. Completeness still requires a complete stable pre/post authority snapshot and no pre-query backfill, so partial/racing lookup always retains `coverageComplete:false`. Recall fuses those persistent hits with the bounded Phase 2A exact-session fallback, while current-agent scope still performs no Archive fallback. Titles distinguish persistent-only, fallback-only, combined persistent-plus-fallback, and vector-only results.
