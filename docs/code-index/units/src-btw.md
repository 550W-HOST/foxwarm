...
- Persists results via `sessionManager.appendSessionMessage` using the display-only message pattern, ensuring BTW output is excluded from model context, compaction, and tool-based session previews. When the BTW LLM call reports `modelId`, that provider-prefixed id is copied to the persisted display-only model message's `__meta.modelId`.
