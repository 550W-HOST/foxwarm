...
- **Stateful Responses WebSocket**: `providerType: openai-ws` uses the Responses request/event format over one provider WebSocket per active chain. Every call begins from a complete final request plan. Rolling SHA-256 fingerprints cover every request invariant except transport-owned `input`, `previous_response_id`, and `stream`, then advance at exact input-item boundaries. An idle candidate must match its secret-safe connection identity and exact prefix; longest-prefix and then newest wins. The wire request sends only the remaining suffix with `previous_response_id`, while unmatched requests send the full input. Completed sockets remain leased until the exact canonical assistant message commits, then the existing serializer projects that message back to provider items before releasing the chain. Refusal responses and malformed function calls without a stable provider call ID are conservatively not reused because their canonical replay projection cannot reproduce the original item exactly.
- **Pool/resource boundary**: Busy sockets are excluded from idle matching and retention. Local placement retains at most five completed idle chains and Session-worker placement retains one; release prunes LRU sockets. Idle close/error removes a chain, eviction/abort/failure/malformed/incomplete responses close it, and entries rotate at the provider's 60-minute boundary. Underlying idle sockets are unreferenced so they do not keep the process alive. The pool stores fingerprints and response IDs, never messages.
