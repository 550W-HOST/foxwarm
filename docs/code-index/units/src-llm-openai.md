...
- **Message conversion**: Transforms internal message structures (with `parts` containing `functionCall`, `functionResponse`, `inlineData`) into OpenAI's expected formats. Handles role mapping (`model` → `assistant`), preserves explicit Responses assistant phases across concrete OpenAI model changes, derives the documented compatibility phase only for legacy phase-less model messages, groups tool responses by `tool_call_id`, serializes function call arguments, and renders `MessagePart.system` through Foxwarm XML-ish prompt wrappers instead of legacy `[SYSTEM:]` text.
...
- **Pool/resource boundary**: Busy sockets are excluded from idle matching and retention. Local placement retains at most five completed idle chains and Session-worker placement retains one; release prunes LRU sockets. Every successful canonical assistant append/release starts a fixed unreferenced ten-minute idle timer that actively removes and closes the completed chain even when no later request arrives. Taking a chain cancels its timer, and a later successful release starts a fresh full idle period; stale callbacks cannot close an active or pending-append lease. Idle close/error, LRU eviction, pool clear, and reuse all cancel the timer. Entries also retain the provider's 60-minute absolute age boundary between requests. Sockets remain referenced while connecting, generating, and pending canonical assistant append; only completed idle sockets are unreferenced, and a reused lease is referenced again. Close/error/abort listeners remain active across the pending-append gap so a disconnected completion cannot enter the pool. The pool stores fingerprints and response IDs, never messages.
- **Lifecycle diagnostics**: At normal configured log levels, each physical WS attempt emits one structured dispatch record and, when reached, one completion record; discard and socket-close boundaries emit one record each. Records correlate the optional Session/purpose/logical-request/iteration/attempt identity with a process-local socket ID, fresh/reused mode, append boundary, connection/create timestamps, bounded frame/byte and first-frame/first-content timings, and close phase/code/reason plus age/idle duration where available. Local idle timeout, LRU/age eviction, pool clear, abort/timeout, finalizer discard, and upstream handshake/active/pending/idle close paths use distinct causes. Diagnostics never include request headers, URLs, prompts, raw response IDs, connection fingerprints, or provider configuration secrets and do not alter provider protocol, retry, retention, or journal persistence.
...
### D-openai-responses-assistant-phase

[2026-09-06] A recognized OpenAI Responses assistant message-item `phase` is canonical optional `MessagePart.phase`, not concrete-model-scoped provider metadata. Capture it on each text/refusal part produced by that item and replay an explicit stored value across OpenAI Responses model switches. Do not infer or persist a phase when parsing a provider response that omits or returns an unknown value.

For outbound compatibility with older phase-less history, derive only the wire phase from the containing canonical model message: any Foxwarm `functionCall` part makes missing text-part phases `commentary`; otherwise they are `final_answer`. Explicit per-part phases always win, phase transitions split message items, hosted search metadata and tool responses do not participate in the heuristic, and the heuristic never rewrites history or changes the Session turn-continuation policy.

### D-openai-ws-lifecycle-diagnostics

[2026-09-06] Stateful Responses WebSocket lifecycle logs must distinguish a fresh physical connection from exact-prefix reuse and identify whether a close was upstream or local cleanup without exposing request/provider secrets. Keep log volume bounded to dispatch, completion, discard, and close boundaries; summarize frames and timing instead of logging per-frame content. These diagnostics are process-local observations only and must not become journal, Session, RPC, provider-protocol, or storage schema state.
