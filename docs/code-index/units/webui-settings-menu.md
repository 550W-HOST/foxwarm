...
Renders a compact dropdown settings menu for the web UI that allows users to configure light/dark/auto color mode, input send-key mode, chat display options (tool grouping, usage badges), instance name, tab icon, and provides access to setup and app reload actions.
...
- `GlobalUiSettingsMenuProps` — props interface for controlled chat/input/branding settings and Setup activation
...
- Closes the menu automatically after color-mode change, successful name/icon save, or setup open.
...
- Receives chat/input/branding values and change handlers from a parent layout/app shell. `useTheme` connects only the compact color-mode control to the shared browser-local runtime; instance name/tab icon are server-backed WebUI settings.
