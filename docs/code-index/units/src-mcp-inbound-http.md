...
- `ExternalExecutionContext` — per-transport immutable generated ID/external identity and mutable selected Node (`master` initially), cwd (`null` initially) and selection generation. The Node catalog changes placement/cwd only after exact authorized actions and generation-checked completions.
