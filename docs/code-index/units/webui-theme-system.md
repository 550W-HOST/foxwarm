...
- `BUILTIN_THEMES` contains six immutable built-ins in the reserved `foxwarm.*` namespace, with one source file per built-in. The colored console remains `foxwarm.550a`; the achromatic machine-console variant is `foxwarm.550a-mono` (`550A Mono`).
...
- `componentTreatment` is a small declarative treatment selector, not arbitrary CSS. Both 550A variants select `console`; an exported and reimported manifest selecting `console` follows the same rendering path.
...
- Chevron/rounded thread treatments clip standalone cards, but an expanded historical tool-group card leaves overflow visible so nested members’ existing external usage badges retain their paint and hit targets; the group header keeps its normal rounded-top card geometry.
- Console Tool header surfaces use the tone marker on the header itself, not a status class inherited from an enclosing Tool card. An outer neutral group therefore cannot recolor nested success/error headers; integrated and plate header recipes keep their intentional transparent treatment.
- Timeline hover affordances have direct owners: assistant/tool top actions respond to their own card, thread strokes to their own direct card/button, and image overlays to their own image button. Hovering a nested child still hovers its ancestor and may activate the ancestor’s own affordance, but does not activate unhovered descendants or siblings. Keyboard focus retains the owning controls and visible disclosure; SpecialBlock controls retain their independent hover/focus behavior.
...
- `themeSystem.test.mjs` covers the six-theme registry, built-in validation, canonical round trips, strict rejection, legacy/current selection retention, bounded custom install/conflict/replace/export/delete, reserved IDs, both 550A palettes, and portable console clone equivalence.
- `themeMarkdownStyles.e2e.mjs` covers Default and both 550A console-treatment Markdown/code pairings.
- `nestedThreadHoverOwnership.e2e.mjs` compares actual standalone versus nested Tool header colors across console light/dark, standard light/dark, and header recipes; physical hover and keyboard-focus probes cover nested Tool, CTX assistant, line, ImageItem, and unaffected LaTeX controls.
...
[2026-09-06] V2's visual grammar was expanded before publication rather than creating a migration-bearing V3. Density, surface/header/control/separator recipes—including a borderless role-tinted `plate` header—role-specific geometry and card gaps, label/icon treatment, elevation, and safe procedural patterns complement semantic color. Default retains its previous measurements and composition, while imported manifests use the identical recipe path.
...
[2026-09-06] The established `foxwarm.550a` ID again denotes the colored dense monospace console so existing selections regain its red, green, blue, amber, and violet role differentiation. The achromatic industrial redesign remains available as the separate portable `foxwarm.550a-mono` built-in named `550A Mono`. Existing browser preferences are not rewritten: stored `foxwarm.550a` selections resolve to the restored colored theme, while explicit `foxwarm.550a-mono` selections retain the machine-console palette.
