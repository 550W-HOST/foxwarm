...
- When `executeTools` receives an authoritative passed Session whose identity matches the source context, node routing and isolation permission use that object directly rather than looking it up through the global all-session map. The captured parallel-exec node snapshot follows the same rule. Legacy/direct callers without a valid matching Session retain the ID-based routing and permission path.
