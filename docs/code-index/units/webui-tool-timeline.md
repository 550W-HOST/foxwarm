...
- [2026-09-07] Composition-level card insets must not leave the tool result surface visible as an uncovered strip beside a banded or tab call/header surface. Those painted headers extend through the card inset to both boundaries, then restore the manifest-defined inset around their own content. This is shared geometry for built-in and imported themes, not a theme-ID adjustment. Transparent `plate` and `integrated` recipes intentionally remain inset rather than simulating a full-width colored band.

- [2026-09-06] Tool surface composition uses the manifest V2 `tool*` family for every ordinary completed-tool surface, tag, separator, control, and activity-track segment. Standard applies its established opacity composition; Console consumes `toolSurface`/`toolSurfaceStrong` directly. Errors remain on `danger*`. Default explicitly assigns its historical `success*` values to `tool*`, preserving its appearance without runtime conditionals; V1 files are rejected rather than guessed. Canonical theme contract: [D-webui-theme-final-surfaces](./webui-theme-system.md#d-webui-theme-final-surfaces).
