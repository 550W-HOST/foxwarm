...
- `buildExternalToolAuthorizationRequest()` — constructs a distinct external request from a verified inbound principal; a real execution-session ID may be supplied, but an absent Agent or path fact is never inferred from an internal Session. `targetNode` is absent unless the trusted caller supplies a concrete Node; internal requests retain their existing master default. The cross-module rule is canonical in [D-dispatch-generic-tool-authorization](../threads/tool-dispatch.md#d-dispatch-generic-tool-authorization).
...
The first enabled matching rule wins, including an earlier allow before a later deny. If no rule matches, `defaultAction` applies to internal Sessions; external requests fall back to deny, including with a missing policy file. `call_tool` remains permission-neutral: direct, unified, and ToolScript calls authorize only the same resolved concrete builtin, Node, or MCP identity. See [D-dispatch-generic-tool-authorization](../threads/tool-dispatch.md#d-dispatch-generic-tool-authorization).
