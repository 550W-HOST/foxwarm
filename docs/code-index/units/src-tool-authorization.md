...
- `buildExternalToolAuthorizationRequest()` — constructs a distinct external request from a verified inbound principal; a real execution-session ID may be supplied later, but an absent Agent or path fact is never inferred from an internal Session. The cross-module rule is canonical in [D-dispatch-generic-tool-authorization](../threads/tool-dispatch.md#d-dispatch-generic-tool-authorization).
