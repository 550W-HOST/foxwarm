...
Secondary files: packages/webui/src/components/ChatTimeline.tsx
...
- `PastedTextBlock` — renders the inline preview/count trigger and owns its read-only modal lifetime.
...
- Opening a chip mounts a focus-contained modal with the complete unchanged inner text in a read-only textarea. Copy writes that complete inner text. Escape, backdrop activation, and Close dismiss the modal and restore trigger focus.
...
- A future composer editor may reuse this exact parser and modal presentation, but this unit currently has no draft-editing or file-reference behavior.
