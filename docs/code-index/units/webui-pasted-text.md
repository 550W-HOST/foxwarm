...
- Opening a history chip mounts a focus-contained modal with the complete unchanged inner text in a read-only textarea. Composer blocks reuse the modal in editable mode with Save/Cancel and optional Restore to text. Copy writes the current complete inner text; Escape, backdrop activation, and Close dismiss and restore trigger focus.
