...
- `ChatTimeline` applies the same user-text presentation to ordinary user text parts and complete direct-channel wrappers stored in user `system` parts. Source metadata keeps its existing visibility setting. Genuine structured-system parts, model text, and tool text remain unchanged. Valid pasted content is excluded from heavy-system and lightweight-metadata classification, so examples inside the block neither promote the row nor disappear when user metadata is hidden.
