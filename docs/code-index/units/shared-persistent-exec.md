...
[2026-08-10] Keep persistent exec lifecycle semantics in `PersistentExecManager`, but inject the small target-local process boundary it needs: platform/runtime identity, managed wrapper launch, best-effort process snapshot, liveness, and live working-directory lookup. The native implementation is the production default for Main and CLI Node. Do not turn this seam into a public driver/configuration API or move wrapper scripts, log/status/cwd artifacts, registry reconciliation, formatting, and completion delivery into the low-level backend.
