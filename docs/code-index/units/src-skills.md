...
- The bundled global `foxwarm-maintenance` skill covers Foxwarm-specific data-root resolution, source/data ownership, backup consistency, and upgrade boundaries. Service control and deployment follow the installation's own runbook rather than generic shell recipes.
- `foxwarm-maintenance` links backup/restore and post-upgrade references from `SKILL.md` for on-demand reading; its SQLite chunk helper remains a single-database component rather than a full-installation backup.
- The bundled global `agent-management` skill links explicit reference docs under `skills/agent-management/references/` for Organizer/Executor collaboration patterns, progressive disclosure guidance, and copyable agent `MEMORY.md` templates. Its memory-edit workflow preserves prompt-cache reuse: current-Session edits normally need no snapshot refresh, other Sessions preferably receive a change message, and immediate snapshot rebuilds explicitly target the intended Session.
