...
- The bundled global `web-search` skill replaces the old `ask-gemini` entry (no alias/shim). Its resource script `skills/web-search/web-search.js` prefers OpenAI Responses API web search (`web_search`, GPT-5.5 by default), supports custom base URLs, preserves Gemini `google_search` fallback config, and uses one shared 240-second request timeout for both providers. It can list/copy concrete GPT entries from Foxwarm `models.yaml` into `~/.secrets/web_search_openai_*` without printing API keys. Raw candidate discovery skips virtual routing entries because they own no credentials; canonical routing contract: [model routing](../threads/model-routing.md).
