...
- The bundled global `web-search` skill replaces the old `ask-gemini` entry (no alias/shim) and is fallback-only: do not load or run it when the current model/provider already exposes built-in/native web search, or from an isolated session/environment. Its resource script `skills/web-search/web-search.js` prefers OpenAI Responses API web search (`web_search`, `gpt-5.6-sol` by default), supports custom base URLs, preserves Gemini `google_search` fallback config, and uses one shared 240-second request timeout for both providers. It can list/copy concrete GPT entries from Foxwarm `models.yaml` into `~/.secrets/web_search_openai_*` without printing API keys. Raw candidate discovery skips virtual routing entries because they own no credentials; canonical routing contract: [model routing](../threads/model-routing.md). The fallback boundary is canonical in [D-skills-web-search-fallback-boundary](#d-skills-web-search-fallback-boundary).
...
[2026-08-11] The bundled `web-search` skill is a fallback for recent/public web lookup, not a competing path for provider-native search. Do not load or run it when the current model/provider already exposes built-in/native web search, and do not use it from an isolated session or environment.
