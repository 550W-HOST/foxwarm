...
Secondary files: skills/web-search/web-search.js, skills/web-search/web-search.test.js, skills/web-search/web-search-mcp.js, skills/web-search/web-search-mcp.test.js, skills/mcp-management/SKILL.md
...
- The bundled global `web-search` skill is fallback-only when provider-native search is unavailable. Its trusted-host direct CLI `skills/web-search/web-search.js` remains forbidden from isolated environments. The persistent stdio resource `skills/web-search/web-search-mcp.js` exposes only exact `web_search({query})` under server name `betabot-web-search`; an isolated agent may use it only after exact administrator configuration and allowlisting, with no caller provider/config controls. The adapter sends queries only over child stdin, bounds bytes/output/concurrency/deadline, redacts stable failures, and treats returned web content as untrusted external reference. It is a packaged resource only until an administrator separately configures the MCP server and exact agent rule. The direct CLI prefers OpenAI Responses API web search (`web_search`, `gpt-5.6-sol` by default), preserves Gemini fallback, and uses one shared 240-second provider timeout. Raw model candidate discovery skips virtual routing entries because they own no credentials; canonical routing contract: [model routing](../threads/model-routing.md). The fallback boundary is canonical in [D-skills-web-search-fallback-boundary](#d-skills-web-search-fallback-boundary).
...
[2026-08-11, updated 2026-08-19] The bundled `web-search` skill is a fallback for recent/public web lookup, not a competing path for provider-native search. The direct CLI is trusted-host-only and forbidden from isolated environments. Isolation may use only the exact administrator-configured and allowlisted `mcp:betabot-web-search/web_search` capability; the caller receives no provider/configuration controls and must treat returned content as untrusted external reference.
