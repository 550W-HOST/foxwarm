...
- An authenticated negotiated-v3 CLI Node advertises `externalToolOwner:1`; disjoint external calls are rejected before effect if malformed, mixed with Session/Agent fields, or unsupported. At this stage its explicit external path supports file primitives and `get_default_cwd` using process/selected cwd, without falling back to an Agent directory. `browse_*` and `exec` are not yet external executable paths. An explicit `disconnect()` stops automatic reconnect.
