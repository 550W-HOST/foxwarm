...
- Loaded rows at any tree depth remain expandable until their own bounded child page proves they are leaves. Expanding a child requests only that parent window, so root → child → grandchild and deeper navigation does not require a complete browser catalog mirror; flat and search presentations do not probe hidden branches.
