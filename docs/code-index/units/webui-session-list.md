...
- Context menu provides archive/unarchive, rename, fork, and delete actions via REST API calls (using stored auth token). Delete confirmation offers a default-off recursive descendant checkbox based on the complete canonical relation graph and stays open with the backend error on failure. Archive opens the analogous confirmation only when archiving a session with descendants; no-descendant archive and all unarchive actions remain one-click.
