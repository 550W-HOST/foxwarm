...
- `NodeProviderRegistry` is the authoritative non-master provider resolver after service validation. The authenticated remote provider delegates to `NodesManager.executeTool()`, which retains WebSocket request/result/timeout and cwd-forwarding behavior.
