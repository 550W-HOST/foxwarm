...
- `updateSessionBusyStateForSession`, `updateSessionBusyState` — exact-owner and current local metadata-backed forms of the same busy/busy-start/runtime-clear/persist/optional-notify transition. Exact turn effects can use full owner persistence whose normal save already emits the state notification; legacy metadata-only callers supply the notification explicitly.
