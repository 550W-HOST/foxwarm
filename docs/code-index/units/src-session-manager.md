...
- `setSessionWorkerEnqueueSink` — registers the Session-worker placement sink; when set, the public `enqueueSessionItem` routes every Main-side producer (typed event wrappers, RPC enqueue, inter-session delivery) through the one durable ingress boundary without local hydration/queue/trigger writes, and fails managed sessions closed retryably instead of spawning a worker. Without a sink the local path is unchanged.
- `setSessionWorkerDeleteHandler` / `setSessionWorkerForkSourceProvider` / `setSessionWorkerFenceChecker` — the Session-worker destructive-lifecycle, fork, and fence-lookup seams: delete entry points first let the hook tear down any worker fence (interrupt, graceful stop with handback, durable fence/mailbox removal) and fail closed on lifecycle errors before ordinary local deletion; fork derives a fenced source from the provider's read-only detached snapshot instead of hydrating the authority into Main; parent-relation moves on a fenced child become Main-owned catalog-only writes guarded by the fence checker and never write the worker-owned authority.
