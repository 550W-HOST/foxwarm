...
- **Lazy hydration:** metadata creates explicitly marked lightweight session objects; `getSession` loads the per-session history snapshot exactly once for that placeholder and ignores obsolete frontier fields. Empty history is valid hydrated state and never acts as a load marker. A persisted live record reconstructed after an in-memory miss still hydrates, interrupted legacy upgrades retry explicitly, and an archive-only ID cannot implicitly start a new lifetime.
