...
- `setSessionWorkerDeleteHandler` / `setSessionWorkerForkSourceProvider` — the Session-worker destructive-lifecycle and fork seams: delete entry points first let the hook tear down any worker fence (interrupt, graceful stop with handback, durable fence/mailbox removal) and fail closed on lifecycle errors before ordinary local deletion; fork derives a fenced source from the provider's read-only detached snapshot instead of hydrating the authority into Main.
