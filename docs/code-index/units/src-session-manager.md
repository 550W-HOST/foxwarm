...
- `saveSession`, `saveSessionForSessionCritical`, `saveSessionCatalogEntries`, `saveSessionCatalogProjectionStrict`, `loadSessions`, `listSessions`, `getAllSessions`, `getSessionCatalog` — persistence and enumeration. `saveSession` remains best-effort; the exact-owner critical form exposes the existing authority save lane and postcommit catalog-resync distinction to the narrow pre-send prompt-snapshot effect. Main local-owner full authority→catalog compositions serialize through one bounded per-Session save lane with cleanup after success or rejection; Worker direct state-file writes remain independent. Normal saves commit authority before an exact row/batch catalog projection. Standalone detached release builds its row inside the metadata lock by overlaying staged semantic authority onto the latest live catalog/presentation fields, preserving catalog-only writes ordered before that lock. Fenced Main-only presentation writes preserve the current semantic projection, while Worker handback explicitly commits one complete bounded projection. `getSessionCatalog` is a Main-owned loaded-stub read that never hydrates worker authority.
...
- `applyQueuedItemToWaitState`, `enqueueSessionItem` — shared pure wait/drop/defer/enqueue transition plus canonical Main insertion with managed-inbox handling. Local ingress joins an open standalone-compact admission before hydration/mutation; ingress already admitted before release finishes under busy, while release-window ingress waits and then resumes through the ordinary persistence/trigger rule after finalization opens the gate.
...
- `updateSessionBusyStateForSession`, `updateSessionBusyState` — exact-owner and current local forms of the same busy/busy-start/runtime-clear/persist/optional-notify transition. The current local wrapper uses the critical authority-then-catalog save rather than a catalog-only projection write. Persistence rejection normally restores the exact prior busy fields without clearing runtime state or notifying; when authority committed and catalog projection failed, the local wrapper retains the committed/resynchronized fields and propagates the postcommit failure. Successful release durably clears `busyStartedAt` before runtime state is cleared. Standalone compact finalization acquires the same save lane, commits a detached idle snapshot while live remains busy, then patches only release-owned live fields; concurrent semantic setters wait and later persist their mutations together with committed idle fields. Exact turn effects use their full owner persistence; callers that intentionally provide another persistence boundary own its notification and rollback predicate explicitly.
...
- `requestSessionCompaction`, `cancelSessionCompaction` — manual compaction and compact-only cancellation. Idle local sync compaction owns the transient ingress admission through staged durable release; the shared live owner remains busy until committed idle state is adopted.
- `requestSessionStop`, `requestSessionDequeue`, `retrySession` — current run/queue controls; ordinary Stop rejects the standalone compact owner before hydration, while other controls retain their normal busy-based semantics because staged finalization never exposes precommit live idle. Retry delegates directly to the router without queue persistence.
...
- `forkSession`, `createChildSession`, `resolveSpawnedSessionModelEffort`, `setSessionParent`, `updateChildSessionParentIds`, `getChildSessionIds`, `getCanonicalChildSessionIds`, `collectSessionDescendants` — lineage, one-pair model/effort spawn inheritance, explicit parent links, and canonical lifecycle traversal. Spawned children persist only the resolved current pair and leave their own future-child defaults unset. Fresh cross-Agent children use the target namespace, target memory snapshot, and target isolation binding while retaining the caller parent; cross-Agent forks fail before creation effects and same-Agent behavior is unchanged. Fork/child creation rejects a claimed source or parent at lifecycle entry before detached-provider, hydration, prompt-cache, or allocation effects, then rechecks before the child commit. `forkSession`/`createChildSession` accept a trusted `sourceOverride` snapshot so the Main management facade can derive a worker-fenced parent from a strictly read-only detached authority read without hydrating or persisting it.
...
- `createAgentWithMainSession`, `createSessionInAgent`, `moveSessionToTarget`, `setAgentInherit`, `setAgentIsolation`, `deleteAgent`, `refreshSessionSnapshot` — façade over agent operations. Named session creation accepts an optional semantic Node; target isolation still takes precedence. Inheritance mutation defaults to metadata-only and an opt-in batch refresh skips Sessions inactive for more than one hour before hydration. Agent creation refreshes only its exact newly created or converted main lifetime after applying inheritance. Agent creation and isolation mutation accept optional exact `toolRules` replacement. Under Worker placement, creation of new lifetimes remains Main-owned; same-source agent creation may consume one matching detached read-only source override for inherited model/node/agent-memory identity without hydrating the catalog stub. Existing-session identity conversion/move, another-source creation, and agent-wide snapshot-affecting inheritance/isolation/deletion changes fail before effects until a closed ownership path exists. Rule-only replacement is the narrow exception when the requested isolation binding is unchanged because it mutates only Main-owned agent metadata and Workers refresh that exact entry before authorization. Agent deletion rejects `main`, inherited-by targets, and active owned Sessions; exact typed confirmation authorizes destructive removal of idle owned Sessions including queued work through canonical Session deletion, which retains archive identity reservations before removing the Agent directory and metadata.
- `getAgentMetadata`, `getAgentToolRules`, `getAgentInheritanceChain`, `getAgentIsolationNode`, `isAgentIsolated`, `isSessionEffectivelyIsolated` — exact agent metadata access; tool rules do not follow inheritance.
...
- **Lazy hydration:** metadata creates explicitly marked lightweight session objects; `getSession` loads the per-session history snapshot exactly once for that placeholder and ignores obsolete frontier fields. Empty history is valid hydrated state and never acts as a load marker. A persisted live record reconstructed after an in-memory miss receives the same placeholder identity until replacement succeeds, interrupted legacy upgrades retry explicitly, and missing/invalid authority fails closed without exposing default semantic state. An archive-only ID cannot implicitly start a new lifetime.
...
- **Critical persistence:** creation/move paths and local busy ownership transitions use strict authority-then-catalog writers and roll back known precommit failures, including partial archive appends. Authority-postcommit catalog failures retain/resynchronize committed semantic state and propagate. Ordinary runtime saves remain best-effort where callers historically do not handle persistence errors.
- **Startup recovery:** pending identity-move recovery runs outside the ordinary best-effort load catch. After startup migrations, `loadSessions()` explicitly awaits archive-store initialization so reservation-ledger repair and ledger/SQLite conflict validation finish before normal agent/channel/session loading becomes available. It also loads agent-metadata authorization authority before the broader recoverable channel/session boundary; malformed persisted tool rules reject readiness instead of continuing with an empty non-isolated map. Journal, reservation, metadata-authority, or recovery failure rejects initialization so partially recovered state is never loaded as a normal session stub.
...
- **Child/fork creation:** copies or rebuilds prompt snapshots/cache lineage according to fork semantics, records archive lineage, awaits best-effort derived Vector/lexical baseline initialization from the committed target fork caps before suffix append, and advances suffix counters past retained IDs. Failed creation first rolls back Archive authority and then best-effort clears the target's derived index state before that ID may be reused.
...
- `src/nodes/remoteExecLiveness.ts` owns transient Main-process remote exec reservation/claim state and receives narrow successful-move/delete lifecycle hooks.
...
- Lazy load upgrades only unversioned per-session files by seeding historically catalog-only fields, while current-format files exactly replace semantic stub state. Hydration eligibility is explicit placeholder/reconstruction/upgrade state rather than `history.length`, so reads of a legitimately empty live owner cannot roll it behind concurrent mutations. Local and Worker placement keep the SQLite catalog Main-owned; Session-worker placement supplies bounded projections through SessionRuntime handback.
...
- Session deletion clears runtime/pending compact state, transient remote-exec reservations/claims for the canonical ID and aliases, the live map, attachments, session/legacy-frontier files, and shared metadata. A successful local identity move rebinds matching transient remote-exec Agent ownership while preserving the original capability-bound Session ID for completion. Session history clear also removes any armed wait. Deletion currently leaves archive store/log and vector data intact; canonical scope is documented in [session lifecycle](../threads/session-lifecycle.md#archive-and-deletion).
