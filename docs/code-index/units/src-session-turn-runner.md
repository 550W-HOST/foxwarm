...
4. `llm.chat` performs provider retries/journaling and appends its assistant result. Tool calls enter the existing `llm.executeTools` path with current-turn QQ reply metadata in the in-process ToolContext; the complete tool result is appended before wait/compact/follow-up decisions. A no-tool result also checks one compatible leading queue prefix before delivery; consumed input stays in separate canonical rows, runs the shared usage-threshold compact guard, and continues only after the loop-top compact safe point.
