...
Holds Main-process-only authority for real authenticated CLI Node and first-party resident Docker-worktree persistent commands started by a verified external identity in one generated inbound context. Records are not Foxwarm Session receipts or recoverable persisted owner state.

## Key functions

- `reserveExternalExec` allocates a canonical real petname ID and existing signed completion capability before effect. For a remote Node, the signature binds its authenticated ID, external identity, context UUID and exact exec ID; resident Docker retains the same record/capability only inside Main and separately binds its real provider generation before process launch. At most 20 records and 64 KiB of original arguments per record are retained for later policy checks. Once full, a new reservation evicts the oldest completed record; 20 unresolved records deny a new effect rather than discarding live ownership.
...
The authenticated remote Node owns its logs/status behind WebSocket result requests. The resident Docker provider uses its same-generation Main-owned `ExecRuntime` to read bounded partial/final output; external file calls cannot read its artifact directory. Losing Main's map makes old results unavailable even if the provider retained artifacts. Rule revocation is applied by the caller on each result query. The canonical cross-module contract is [D-node-thread-core-protocol-compatibility](../threads/node-communication.md#d-node-thread-core-protocol-compatibility).
