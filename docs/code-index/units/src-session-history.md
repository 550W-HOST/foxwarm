...
- The planning model preserves the live prompt-cache key on its clone because pre-commit prefix/schema lineage is unchanged. The engine separately marks only planning that is actually running in the background; awaited planning, including the missing-enqueue synchronous fallback, remains unmarked. A selected `openai-ws` leaf consumes that transient distinction under [D-model-routing-openai-ws-prompt-cache-key](../threads/model-routing.md#d-model-routing-openai-ws-prompt-cache-key), without mutating the clone or live persisted lineage.
...
- The shared awaited engine asks its exact local or Worker owner to publish the existing compaction runtime phase before prompt-cache preparation and holds it through provider retries and commit. Success, failure, cancellation, and no-op paths release only that owned phase; background planning never requests foreground publication.
