...
- Active browser composition owns its live DOM and Selection: interim composition input never reconciles/removes caret anchors or emits partial canonical drafts. Finalization waits for the committed input after `compositionend` when present, with an animation-frame fallback for browsers that deliver final input first; it then promotes anchor text once, emits once, and records one composition undo group.
