...
`NodeToolRequest` admits a disjoint explicit external-owner branch for Main-local verified inbound callers without `sourceSessionId` or Agent. The authenticated CLI Node provider handles that branch over negotiated v3; the first-party resident Docker-worktree provider alone also opts in to owner-aware primitive files and exec through an in-process marker. Main-local call options carry a live-context fence to remote final send or resident primitive/process launch, plus an exact Docker generation binding for retained results. Other primitive providers fail before effect, and the internal Session request shape and provider behavior remain unchanged. Multiple primitive calls in a file mutation cannot turn an already-attempted effect into a false pre-effect error.
...
- `NodeFilesystemRequest` / `NodeExecRequest` — fixed primitive filesystem and complete exec requests carrying either a real internal source Session or disjoint verified external owner, plus exact Node and bounded routing context. `FIRST_PARTY_DOCKER_EXTERNAL_OWNER` is an in-process marker for the resident first-party primitive adapter, not a remotely advertised protocol feature.
...
- The first-party Docker marker and `kind/type` check are required before an external owner reaches primitive composition. External file requests never carry a synthetic `sourceSessionId` or Agent; each primitive receives the live-context fence, and a failed operation after an attempted mutation reports an uncertain/partial effect rather than claiming that nothing ran. Executable-provider protocol v1 and all other primitive adapters remain external-owner unsupported.
