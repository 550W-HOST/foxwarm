...
- `callExternalNodeTool` binds the verified identity and immutable context UUID, authorizes exact source `node`, target Node, full arguments and execution-derived file/patch/cwd path facts, then dispatches to the actual advertised capability. Exec reserves one real persistent exec ID and Main-signed capability before dispatch; error/timeout paths do not retry effects. Foreground and background results stay scoped to the original context, including after current-Node changes.
...
- `externalNodeAction` authorizes the concrete `builtin:node` action and checks actual compatible capabilities for list/status/select. A new selection uses the Node-supplied default cwd rather than another Node's working directory. Completed commands update cwd only if the selection generation is unchanged.
- `releaseExternalNodeContext` discards Main's live ownership records on DELETE/expiry/shutdown and sends best-effort Node release. It does not kill running processes. Node-local artifacts are removed after the managed process exits.
