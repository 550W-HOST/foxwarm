...
- Outbound messages are sent through the latest active WeWork card for a source-bound conversation, HTTP `response_url` when explicitly bound, WebSocket `aibot_send_msg`, or a configurable legacy/group webhook URL. Ordinary proactive text and files prefer enabled WebSocket delivery unless the call supplies an explicit webhook override.
