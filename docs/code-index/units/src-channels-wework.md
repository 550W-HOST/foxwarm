...
- In WebSocket mode, ordinary file/image sends use bounded 512 KiB sequential temporary-media upload commands and proactive delivery to the same target; captions are separate same-route text messages after upload. Dual configuration prefers WebSocket unless the call supplies an explicit `webhookUrl`, and selected WebSocket delivery never blindly retries through the webhook. See [D-channel-wework-aibot-file-delivery](../modules/channels.md#d-channel-wework-aibot-file-delivery).
