...
- Compatible input that arrives while a no-tool provider request is in flight is consumed at Router's pre-final safe point. The intermediate canonical model result is not delivered and does not finish the latest card; the continued provider response owns the visible final.
