...
| `WeWorkWebhookChannel.maybeAggregateStreamMessage(userId, text, options)` | late | Applies broadcast text or `options.channelTurnProgress` to explicit `options.weworkStreamId`; never uses only “latest conversation” |
...
| `WeWorkStreamAggregator.begin(conversationId, delivery, streamId?)` | helper | Starts a new stream and indexes it by stream id; latest conversation pointer is only advisory |
...
- Outbound messages are sent either through an explicit active WeWork stream id, WebSocket `aibot_send_msg`, or a configurable legacy/group webhook URL.
...
- When `aibot.stream: true` and an intelligent-bot callback (`response_url`/`aibotid` or WebSocket callback) arrives, the channel starts a stream aggregator and stores its `streamId` on the `ChannelContext`/queue source. The initial stream content is a quoted `🤔 thinking` status line. Subsequent broadcasts append by explicit stream id, not by current/latest conversation, so busy/queued concurrent turns do not write to the wrong card. Stream-bound broadcasts that reach another WeWork conversation/channel are skipped instead of falling back to legacy webhook send.
...
- If the session is already busy, the inbound callback can still return a new passive stream card immediately; the router must retain that queued item’s `weworkStreamId` until it becomes the next turn source so final/progress broadcasts update and finish that card instead of leaving it at the initial thinking line.
...
- In webhook/short-connection mode, the initial callback returns a passive `msgtype: stream` payload and later `msgtype=stream` refresh callbacks pull the latest aggregate snapshot. In WebSocket/long-connection mode, the channel pushes each stream update with `aibot_respond_msg` using the callback `req_id` and waits for command acks.
...
- The channel exposes `weworkStreamId` on `ChannelContext`; `MessageRouter` persists it in `QueueSource`, and `session/channels.createSessionBroadcast()` passes it back to the WeWork channel so stream updates are turn-bound rather than conversation-active-bound.
