...
- Outbound messages are sent through the latest active WeWork card for the attachment conversation, the bounded adapter-local latest HTTP `response_url` for one non-stream terminal attachment delivery, WebSocket `aibot_send_msg`, or a configurable legacy/group webhook URL. The adapter takes a terminal `response_url` before awaiting its send and does not cross-route after failure; ordinary proactive text and files prefer enabled WebSocket delivery unless the call supplies an explicit webhook override, which remains higher priority than the stored callback.
