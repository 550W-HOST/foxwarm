...
- Loads sessions and completes startup migrations before starting the configured local/child vector owner, then initializes the local SessionRuntime service and ensures a "main" session exists. Enabling the not-yet-implemented session child placement fails startup explicitly.
...
- On `SIGINT`/`SIGTERM`, stops SessionRuntime event publication and drains its accepted local calls before gracefully draining the vector owner and exiting.
