...
- On `SIGINT`/`SIGTERM`, shuts down Session workers with bounded handback, closes the Main process's lazy ToolScript/Monty pool before its host-call services, terminally drains Node execution, MCP, Main Management, and SessionRuntime calls/publication, gracefully drains the vector owner, closes the Journal store/pool, and exits.
