...
- Loads sessions and completes startup migrations before starting the configured local/child vector owner. With `sessionWorkers:true`, it opens the durable Worker store, reconciles ownership, creates the supervisor/ingress coordinator, initializes Worker-aware SessionRuntime, installs enqueue/delete/fork/fence hooks, and resumes pending mailbox work; Session-semantic reads ensure/load an owner before detached authority reads. With workers disabled, SessionRuntime remains local. Main Management and Node execution initialize after SessionRuntime, then startup ensures a `main` session exists.
