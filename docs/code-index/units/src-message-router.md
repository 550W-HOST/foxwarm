...
| `MessageRouter.drainLeadingQueuedMessageParts(session)` | ~152 | Pops/merges leading part-only queue items but stops before compaction/retry controls or a different WeWork stream id so concurrent cards don't mix |
| `MessageRouter.consumeLeadingQueuedTurnInputs(session, pendingParts, turnStreamKey?)` | ~170 | Drains non-compaction/non-retry queued user inputs before the next in-turn LLM call until a different stream/card boundary is reached |
...
- Independently of channel progress, `runSessionTurn` sets canonical transient runtime state for session list/status: `requesting-model` immediately before `llm.chat`, batch-level `running-tool` before tool dispatch, and per-tool `running-tool` details via the tool executor's `onToolStart` callback. All active runtime state is cleared when session processing finishes or when queued compaction exits.
...
- Top-level queued turn start preserves WeWork stream/card boundaries: stream-bound queued messages with different stream ids, or stream-bound vs unbound messages, start as separate turns instead of being merged into one LLM request/card.
- Once a tool loop turn is already in progress, each pre-LLM safe point drains mergeable non-compaction/non-retry queued inputs into the next LLM call. This preserves the queue-unification behavior where user follow-ups sent during tool execution can be seen by the next model iteration, while keeping queued WeWork messages with a different stream/card id in the queue for their own next turn/card update. Internal `/retry` queue controls stay as their own turn boundary and run with queued-input draining disabled for that turn, so later user items are processed only after the retry turn completes.
...
- Concurrent first messages for one unbound channel/conversation use the session manager's keyed get-or-create boundary and converge on one attachment without orphan lifetimes. Random single-session guest names allocate inside the identity lock; inherited guest-agent generation retries directory, live-ID, and archived-main collisions. Canonical semantics: [D-lifecycle-archived-id-reservation](../threads/session-lifecycle.md#d-lifecycle-archived-id-reservation).
