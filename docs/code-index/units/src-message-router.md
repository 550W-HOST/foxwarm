...
- Top-level queued turn start preserves platform reply boundaries: WeWork stream/card messages with different stream IDs, and QQ Bot messages with different passive-reply IDs, start as separate turns instead of being consumed by one LLM request/card. Compatible queued inputs and structured events may share one provider request, but each is appended as its own canonical history message first.
...
- Once a normal or manual-retry tool loop turn is in progress, each pre-LLM safe point drains compatible ordinary queued inputs into the next LLM call. This preserves the queue-unification behavior where user follow-ups sent during tool execution can be seen by the next model iteration, while keeping ready compact commits, differing WeWork stream IDs, and differing QQ Bot passive-reply IDs as safe/source boundaries. The router flushes the unsent current input and appends every consumed queue item separately before that request; provider serialization owns any necessary adjacent-role normalization.
