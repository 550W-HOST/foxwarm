...
- A no-tool provider result is already canonical history when Router performs one final compatible leading-queue consume before final delivery. Matching input that arrived during the request is appended as its own user message and causes another provider iteration. Before continuing, Router broadcasts non-empty result text once as a non-final intermediate using the shared model-intermediate channel options (Markdown, WebUI excluded, current WeWork stream excluded, conversation-latest QQ context); empty text remains unsent. A different source boundary remains queued and the current result proceeds as an ordinary final. Router also runs the same usage/effective-threshold auto-compaction request guard as the tool-loop path, letting the loop-top compact safe point apply a pending commit before the next provider call without replaying either input.
