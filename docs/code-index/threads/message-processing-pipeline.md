...
8. When function calls exist, router publishes structured progress and calls `llm.executeTools`; normal dispatch resolves builtin, MCP, or node tools through permission-aware tool infrastructure. If the provider result also contains non-empty model text, the local runner broadcasts it before tool execution; a Session Worker uses the fixed Main intermediate-delivery operation after the canonical model append/publication, excluding WebUI and WeWork stream aggregation just like local mode.
...
10. After a no-tool result has been appended canonically, the runner performs one compatible leading-queue consume before final delivery. Matching input that arrived during the provider request is appended as its own canonical message, the completed non-empty result is delivered once as intermediate text, and the provider loop continues; otherwise the result is sent through direct source reply/session broadcast with `turnFinal:true`. A Session Worker uses the fixed Main intermediate or committed-final service only after the canonical response append and full projection acknowledgement; transient progress remains separate. When a tool/wait/stop path already delivered intermediate model text, the runner preserves local `lastTextBroadcasted` behavior and does not send that same text again as a final response.
...
When a canonical model result contains non-empty text and the same runner invocation will continue—because it contains tool calls or because a compatible queued follow-up arrived during the request—the text is published once as intermediate output. Session-worker placement performs one fixed Main delivery attempt to external channel attachments after canonical persistence/publication. The typed operation carries the serialized source so QQ uses its current/latest passive message ID and monotonic sequence; WebUI and the active WeWork stream aggregation remain excluded, and `turnFinal` is not set. Delivery failure is logged without poisoning, rollback, retry, or outbox. A later genuine no-tool terminal result uses committed-final delivery, while tool/wait/stop completion preserves the local `lastTextBroadcasted` duplicate-suppression behavior.
