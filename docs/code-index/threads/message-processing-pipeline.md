...
- `/retry` atomically claims an idle session and enters the ordinary turn loop directly with `parts:null`; it does not persist queue state, regenerate a completed answer, or add a model-facing retry marker. Under Session-worker placement, slash and WebUI callers ensure the exact Worker owner and invoke that same canonical retry entry with serialized source/finality metadata; no Main stub mutation, mailbox row, or second queue exists, and active overlap rejects definitely rather than waiting behind the current turn. If the Worker response is lost after invocation, the caller reports an unknown outcome that may already be committed/delivered and tells the user to inspect history; it never labels that state a definite failure or auto-retries it. Compatible input arriving after the claim joins at normal safe points.
