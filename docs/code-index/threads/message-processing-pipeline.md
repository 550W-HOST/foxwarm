...
8. When function calls exist, the runner publishes structured progress and calls `llm.executeTools`; normal dispatch resolves builtin, MCP, or node tools through permission-aware tool infrastructure. If the provider result also contains non-empty model text, the local runner broadcasts it before tool execution; a Session Worker uses the fixed Main intermediate-delivery operation after the canonical model append/publication, excluding WebUI and WeWork stream aggregation just like local mode.
