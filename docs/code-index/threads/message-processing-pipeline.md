...
`waitAllSessions` is an optional all-session report barrier, not the ordinary way to wait for one child. Omission and an explicit empty array are exactly equivalent ordinary waits. Some model/provider paths may emit every declared argument even when optional, despite non-strict schema use, so `waitAllSessions: []` is a supported empty placeholder and must normalize to omission. A non-empty value must normalize, after trimming and deduplication, to at least two distinct session IDs; use it only when the next step strictly requires a new report from every listed session. Other activity or a timeout may still wake the session with the barrier pending. Historical persisted/history classifiers may continue treating omitted or empty lists as a bare wait; no migration or field rename is required.
