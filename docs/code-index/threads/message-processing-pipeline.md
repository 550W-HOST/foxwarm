...
4. The registered trigger invokes `MessageRouter.processSessionQueue`, which directly delegates to its `SessionTurnRunner`. One bound local turn-effects owner carries the exact Session through save, canonical append, busy/wait, and runtime/history events. One outer processor acquires the reentry guard and rollback-safe persisted busy claim once, then iteratively selects the next owned action: requested retry, leading compact commit, or the complete leading ordinary queue prefix. It performs the ordinary busy release once after the selected actions drain. Only a successfully completed processor may trigger the finish-window trailing handoff; failure in that intentionally spawned processor is logged once without retrying or consuming its durable queue.
...
8. After each provider result's model row is persisted/published, the Worker host ingests newly durable mailbox input and the runner checks the leading ordinary queue prefix. The iteration computes `willContinue = hasTools || hasQueuedInput` and performs exactly one model-text delivery: intermediate when true, committed final when false. Empty terminal results emit a generic lifecycle final that only capable adapters consume; WebUI stream presentation remains separate.
9. When function calls exist, the runner publishes structured progress and calls `llm.executeTools`; normal dispatch resolves builtin, MCP, or node tools through permission-aware tool infrastructure. The complete tool row is appended before any queued user rows, so a function call/result pair is never split. A successful `afterSend:"wait"` handoff arms the existing activity wait only after that row is canonical, while `afterSend:"finish"` stops the turn without creating wait state.
10. Queued rows append after the provider text decision and, when tools exist, after the tool row. Tool wait/Stop/managed/tool-stop terminals close active WeWork aggregation when needed without resending model text. Continuing iterations retain the existing compaction and goal-reminder safe points.

Ordinary queue prefixes preserve one canonical Message, sequence, and client identity per logical input but commit the selected prefix in one authoritative append. Child-handoff mutations retain queue order and roll back with a precommit failure; Stop uses the same batch boundary for each finalization pass. This prevents an authority snapshot from dropping the whole selected prefix while only a strict subset has entered history.
...
- `/continue` enters the ordinary turn loop directly with `parts:null` only for a derived interrupted idle turn. The public `/retry` command is removed; internal SessionRuntime/RPC retry names remain implementation details. Exact-owner history validation prevents continuation after a completed answer. Under Session-worker placement, slash and WebUI callers ensure the exact Worker owner and invoke that same canonical entry without serialized reply-target metadata; no Main stub mutation, mailbox row, or second queue exists. If the Worker response is lost after invocation, the caller reports an unknown outcome that may already be committed/delivered and tells the user to inspect history; it never labels that state a definite failure or runs it automatically again. Ordinary input arriving after the claim joins at normal safe points.
...
- Ordinary queued continuation is iterative under that one processor claim; a turn never recursively invokes a later outer turn.
...
### D-pipeline-source-blind-turn-routing

[2026-09-21] Session queue consumption and automatic model output are source-blind. The runner consumes the complete leading ordinary queue prefix in canonical order, retaining one canonical Message per logical input, and does not split by platform, configured Channel instance, conversation, card, message ID, or reply preference. Real compact, Stop, wait, managed-control, tool-call/result, mailbox/authority, and lifecycle boundaries remain unchanged. `${TURN_ID}` correlates one provider/tool lifecycle and presentation progress only; it is not a reply target.

Intermediate, final, error, and empty lifecycle output goes to every eligible attached Channel through the same local or Worker attachment path. WebUI retains its independent history/stream presentation exclusions. Empty finals invoke only adapters with a lifecycle handler, so platforms that do not need completion receive no empty message. QQ Bot and WeWork choose passive/proactive delivery from their own bounded latest per-conversation context; current QueueSource, Worker RPC, and tool context carry no card/message receipt or reply-control metadata. Request-level slash, authorization, and managed acknowledgements remain immediate `ctx.reply` operations and are not model output.

### D-pipeline-canonical-queue-item-boundaries

Each logical queued input or event is appended as its own canonical `Message` in queue order, including `parts` items and structured `message` items. The ordinary prefix may be consumed before one provider request so tool-loop follow-ups affect that request, but the runner never concatenates queue-item parts. Ready compact commits and the established lifecycle/control boundaries remain queue boundaries; Channel source changes do not. Provider-specific serializers, not persisted history, normalize adjacent same-role messages when a protocol requires it.
...
Retries are observable but not model-visible. One updatable display-only notice plus channel snippets is the preferred presentation. Each emitted notice starts with the compact single-line form `⚠️ LLM Error: Attempt …`; accumulated WebUI attempts retain one newline only between attempts, and embedded status/reason whitespace is collapsed so it cannot split an attempt. Within one physical request, a consecutive retry with the same bounded status-and-reason descriptor renders `(same error)` in the display notice and channel snippet while retaining the actual event metadata. Ordinary channels receive only the first and terminal-failure snippets; retry presentation does not create a source-targeted exception. WebUI remains history/SSE-only for this presentation.

### D-pipeline-worker-intermediate-model-text

Each canonical provider result decides its own text finality after persistence/publication and the ordinary-input peek. A result that has tools or queued input is published once as intermediate output; a terminal result uses committed-final delivery in the same iteration. Session-worker placement performs one fixed Main attachment-delivery attempt. The typed operation carries no reply source; WebUI remains excluded from intermediate delivery, `turnFinal` is not set, and each Channel independently uses its latest passive context when appropriate. Delivery failure is logged without poisoning, rollback, retry, or outbox. Tool wait/Stop/managed/tool-stop terminals close active WeWork when needed and never resend that model text; no unified tail response or duplicate-suppression flag remains.
...
[2026-08-27] `send_to_session` and `create_child_session` expose one optional `afterSend` enum that separates delivery from turn termination: `continue` sends and continues the current turn (default), `finish` sends and stops the turn idle without wait state, and `wait` sends, stops, and records the resolved target as an expected `waitAnySessions` source. Completed child reports use `finish`; `wait` is reserved for a handoff that genuinely requires later activity. Hidden `waitAfterHandoff` and `noFurtherAssistantReply` inputs remain runtime-readable only for old direct/persisted callers and cannot be combined with `afterSend`.

After successful delivery, the handler emits hidden post-batch control. Child creation awaits an initial send for both terminal modes, and `wait` requires that message to be non-empty; default `continue` retains asynchronous initial delivery. The runner first appends the complete tool-result message and finishes tool progress, then either stops idle or persists one activity wait. Wait delivery is not target-filtered and does not wait for task completion. Fast replies already queued before arming are consumed immediately after the waiting handoff stops. Multiple successful waits coalesce; failed sends contribute no wait target; a successful waiting handoff still waits when a sibling tool fails. A successful `finish` handoff likewise remains terminal when a sibling tool fails, while creating no wait state. If an ordinary explicit wait loses its stop signal because a sibling tool failed, it clears only the wait token it created so no stale wait remains.
