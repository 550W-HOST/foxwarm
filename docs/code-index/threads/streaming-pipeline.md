...
2. `collectOpenAIResponsesStream` or `collectOpenAIChatCompletionsStream` reassembles provider events, invoking a progress callback on each delta and optional raw-capture callbacks. Responses WebSocket JSON frames enter the same Responses accumulator as SSE events, so downstream progress semantics do not diverge by transport.
