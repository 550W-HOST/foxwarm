...
A worker applies only an ordered session-local mailbox prefix. The caller supplies a bounded count, while persistence reads canonical intent rows and payloads directly from SQLite before invoking apply. The activated child runtime accepts only current QueueItem payloads, preserving each source/item boundary, then runs the shared queue processor against the same owner. It durably replaces JSON through temp write, file fsync, rename, and directory fsync, and only then marks the prefix applied and advances SQLite's acknowledgement cursor. Later turn saves reverify generation/incarnation without advancing that cursor; a failed run resynchronizes the same hot owner from authoritative JSON before returning, preserving already-durable queued work for retry. Global mailbox IDs may have gaps belonging to other sessions. On recovery, JSON cursor ahead of SQLite means the already-durable prefix is acknowledged without reapplying it; SQLite ahead of JSON is an impossible ordering violation and fails closed. Applied rows may be cleaned later only through the acknowledged cursor.
