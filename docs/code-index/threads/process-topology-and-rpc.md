...
The first closed Main Management tool service also runs through the local RPC transport. Its exact v1 allowlist is inter-session text delivery, direct channel text delivery, agent listing, and timer CRUD. Direct builtins, unified builtin calls, and ToolScript nested calls are current real callers. Requests contain source identity plus existing tool args and responses retain existing structured control metadata; arbitrary builtins and live Session/history/queue/callback values are rejected by construction. Production terminal shutdown fences initialization and new calls before draining accepted work, so the local transport cannot be recreated during later subsystem shutdown. This is not child-to-parent reverse wiring yet.
...
The local canonical tool executor now accepts one authoritative passed Session for common tool preparation: current-node routing, isolated permission evaluation, parallel-exec routing snapshots, and the `set_goal` persist hook no longer require that source Session to exist in the process-global all-session map. The production local caller remains `LocalSessionTurnHost`; this is an in-process ownership seam, not WorkerHost activation, reverse transport, or a claim that other tool handlers are worker-complete.
