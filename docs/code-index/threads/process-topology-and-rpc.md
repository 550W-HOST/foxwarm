...
Main `submitAndRun` accepts one current ordinary QueueItem for an exact canonical session ID, proves an already-ready durable generation/incarnation and matching live Supervisor entry before append, writes one immutable mailbox intent, and calls that exact Worker's existing `runPending`. Concurrent calls rely on SQLite mailbox ordering plus the Worker's existing serialized host; the coordinator does not add a second queue, lock, claim, or turn state machine. A failure after append remains a durable ambiguous submission and never falls back to local execution. A Main-memory-only registry holds the actual live ChannelContext under the complete strict QueueSource only for the duration of one submission; zero or multiple exact registrations resolve to attachment fallback, and all paths clean their registration. This operation is not yet selected by production router/config startup.
