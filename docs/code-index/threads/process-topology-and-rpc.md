...
The first production-routed child-process service on this boundary is the LanceDB/vector owner. SessionRuntime has a production-used local DTO service for high-level session queries, enqueue/control, settings, and update events. Session child placement remains disabled while its supported vertical slice is implemented. Durable ownership/mailbox, supervised child lifecycle, save-before-ack persistence, an activated real-child canonical turn caller, reverse Main services, awaited complete committed state projection publication, exact history presentation, and fixed committed-final delivery now exist; remaining service closure and placement routing are not production-wired yet.
