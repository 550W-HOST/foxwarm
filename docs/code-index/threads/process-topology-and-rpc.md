...
The first Node execution forwarding service is likewise local-only. Direct node-environment builtins resolved to a remote node, explicit dynamic node calls, and ToolScript's existing wrapper share one versioned request containing source identity, a non-master authenticated node, an advertised Node-domain tool name, args, and only the optional node/cwd routing snapshot needed by parallel direct exec. The colocated `master` environment bypasses RPC and invokes the local handler directly. Terminal drain is fenced, but no child-to-main transport is connected yet.
