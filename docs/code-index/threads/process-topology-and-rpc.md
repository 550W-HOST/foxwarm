...
A worker applies only an ordered session-local mailbox prefix, and the apply callback receives the canonical store-read intents rather than caller-provided payloads. It durably replaces JSON through temp write, file fsync, a claim/fence check immediately before rename, rename, and directory fsync, and only then marks the prefix applied and advances SQLite's acknowledgement cursor. Global mailbox IDs may have gaps belonging to other sessions. On recovery, JSON cursor ahead of SQLite means the already-durable prefix is acknowledged without reapplying it; SQLite ahead of JSON is an impossible ordering violation and fails closed. Applied rows may be cleaned later only through the acknowledged cursor.

Workers write only the per-session file. Main remains the sole `sessions.json` catalog writer and consumes a bounded cloned projection rather than retaining a second mutable hot Session. A main-owned catalog coordinator registers projection ownership, preserves the latest worker-owned fields across unrelated full/global saves, accepts only explicit topology/UI/channel updates from stale main stubs, and releases ownership only at an explicit placement boundary. Local save composes the same state-file write with the existing main catalog write.

Main-owned lifecycle work uses a tokenized `AbortSignal` claim that blocks respawn, confirms worker exit, reloads/reconciles into a detached Session, applies the mutation, and validates the claim after reload, after the callback, before state/catalog calls, and immediately before each file rename. Shutdown aborts claims and waits only within its deadline; noncooperative callbacks become stale and cannot commit a late write. Successful main mutation updates authoritative state and catalog projection, then installs only its committed bounded main-owned patch into the real live catalog stub before releasing the claim. Final worker-ownership release is forbidden during a claim and requires a reconciled authoritative Session handoff into that live stub before coordinator protection is removed.
