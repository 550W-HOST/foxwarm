...
The first Node execution forwarding service is likewise local-only. Direct and unified node-environment builtins resolved to a remote node, explicit dynamic remote node calls, and ToolScript's existing wrappers share one versioned request containing source identity, a non-master authenticated node, an advertised Node-domain tool name, args, and only the optional node/cwd routing snapshot needed by parallel direct exec. The colocated `master` environment bypasses RPC and invokes the local handler directly; explicit dynamic master calls and master Node discovery are restricted to the canonical node-environment set. Terminal drain is fenced, but no child-to-main transport is connected yet.

The MCP external service is also local-only. Its fixed v1 methods cover redacted server summaries, optional-server tool discovery, named calls with cloneable args, and managed upsert/enabled-toggle updates. Direct compatibility tools, unified wrappers, and ToolScript nested calls are current real callers; only the handler reaches the authoritative MCP client live snapshot and connection owner. Stored configuration values are never returned, configured secret values are scrubbed from surfaced connection/config errors, accepted calls drain during terminal shutdown, and no child-to-main transport exists at this checkpoint.

The local canonical tool executor now resolves one coherent source owner before common tool preparation. `LocalSessionTurnHost` plus `CurrentSessionEffects` may provide an authoritative detached Session without adding it to the process-global map; legacy/direct calls instead load exactly one existing global source by ID and never trust same-ID clones. Current-node routing, isolated permission evaluation, handler context, parallel-exec node/cwd snapshots, and the `set_goal` persist hook share that owner. This remains an in-process ownership seam, not WorkerHost activation, reverse transport, or a claim that other tool handlers are worker-complete.
