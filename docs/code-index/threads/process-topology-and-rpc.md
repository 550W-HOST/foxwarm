...
The first production-routed child-process service on this boundary is the LanceDB/vector owner. SessionRuntime has a production-used local DTO service for high-level session queries, enqueue/control, settings, and update events. Session child placement remains disabled while its supported vertical slice is implemented. Durable ownership/mailbox, supervised child lifecycle, save-before-ack persistence, an activated real-child canonical turn caller, reverse Main services, and awaited complete committed state projection publication now exist; history/final delivery, remaining service closure, and placement routing are not production-wired yet.
