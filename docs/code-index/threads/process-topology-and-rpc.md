...
Main-owned lifecycle work uses a tokenized `AbortSignal` claim that blocks respawn, confirms worker exit, reloads/reconciles into a detached Session, applies the mutation, and validates the claim after reload, after the callback, before state/catalog calls, and immediately before each file rename. Shutdown aborts claims and waits only within its deadline; noncooperative callbacks become stale and cannot commit a late write. Successful main mutation updates the authoritative state and bounded catalog projection before returning projection ownership to worker placement.
