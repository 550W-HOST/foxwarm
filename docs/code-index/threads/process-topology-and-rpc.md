...
The fixed committed-final delivery service carries only exact Worker source identity, one strictly serialized immutable `QueueSource`, bounded final text, and a fixed response/error/empty-final kind. The canonical Worker runner calls it only after its response/error append and awaited full publication. Main uses an injected full-source matcher for a still-live direct reply only when `preferDirectReply:true`; otherwise it awaits the current attachment selection once, preserving WebUI exclusion, send-only, turn-final, and WeWork empty-final semantics. Business or reverse-transport failure is logged without turn rollback, retry, outbox, or publication poison. Ordinary Main broadcast remains fire-and-forget.
