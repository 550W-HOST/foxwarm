...
The first closed Main Management tool service also runs through the local RPC transport. Its exact v1 model-operation allowlist remains inter-session text delivery, direct channel text delivery, agent listing, and timer CRUD. A separate named `scheduleWaitTimeout` method is a current real caller for the internal post-persistence wait timeout DTO; it is not arbitrary execute or an eighth model operation. Direct builtins, unified builtin calls, and ToolScript nested calls are current real callers of the public execute method. Requests contain bounded source identity/operation-specific data and responses retain existing structured control metadata or a small scheduling acknowledgement; arbitrary builtins and live Session/history/queue/callback values are rejected by construction. Production terminal shutdown fences initialization and new calls before draining accepted work, so the local transport cannot be recreated during later subsystem shutdown. This is not child-to-parent reverse wiring yet.
...
- SessionRuntime child placement still needs reverse global services, committed projection/resync and final delivery, exhaustive supported tool closure, supervisor routing, and idle release. The activated child runner remains disconnected from production placement until that vertical slice is complete.
