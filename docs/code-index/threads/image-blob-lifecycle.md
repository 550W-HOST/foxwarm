...
4. Provider requests clone canonical messages and hydrate referenced bytes only while building OpenAI Responses, OpenAI Chat Completions, Anthropic, or Gemini payloads. Claimed HEIC/HEIF bytes are decoded and normalized there to provider-safe JPEG or PNG without changing the durable blob/reference. Each physical request then sends identical provider-visible image bytes/MIME only once; later occurrences retain model-visible guidance or a `foxwarm-image` descriptor marked `deduplicated="true"`. Request diagnostics redact hydrated payloads.
