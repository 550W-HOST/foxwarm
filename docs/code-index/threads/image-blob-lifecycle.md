...
4. Provider requests clone canonical messages and hydrate referenced bytes only while building OpenAI Responses, OpenAI Chat Completions, Anthropic, or Gemini payloads. Claimed HEIC/HEIF bytes are decoded and normalized there to provider-safe JPEG or PNG without changing the durable blob/reference. Request diagnostics redact those hydrated payloads.
