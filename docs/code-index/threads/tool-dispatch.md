...
9. Direct or unified node-environment builtins resolved to a non-master Node and dynamic Node-domain calls enter the fixed v1 Node execution facade. Main-local callers use local RPC; a Session worker uses its borrowed reverse client with an exact source fence. Main then resolves the exact Node through the provider registry and passes one complete capability request to that provider. The authenticated remote provider delegates unchanged to `nodesManager` and the authenticated Node connection. A startup-configured executable provider instead uses one bounded `foxwarm-node-provider@1` JSON stdin/stdout process per list/invoke request and may advertise multiple sandbox-kind Nodes without WebSocket transport. Dynamic `node:master/<tool>` calls bypass RPC only for the canonical node-environment set and use the local named handler. The service rejects `master`, stale sources, unavailable Nodes, isolation-binding violations, and names not currently advertised by the selected provider. Unsupported capabilities never fall back to master. Old authenticated-node image-result shapes are adapted only at that provider's remote response ingress under [D-node-thread-tool-result-compatibility](./node-communication.md#d-node-thread-tool-result-compatibility).
...
Startup configuration may add zero or more trusted executable providers. Each configured command and fixed argument list is host authority, never model-mutable, and is launched one-shot with `shell:false` for either `list` or `invoke`. The fixed protocol is `foxwarm-node-provider@1`: one bounded JSON request on stdin, one bounded JSON response on stdout, exact protocol/provider/request/operation identity, sandbox-only descriptors with provider-defined type/availability/tools/default cwd, and a complete invocation carrying exact source session, agent, Node, arguments, and authoritative routing context. Executable Node IDs use the canonical slash-free bounded ASCII grammar and cannot claim the reserved master ID. It carries no mutable Session, callbacks, provider secrets, or Main credentials. Malformed/multiple/oversized responses, descriptor/schema violations, unavailable capabilities, provider errors, abnormal exits, timeout, cancellation, or unclosed inherited stdio fail without retry or master fallback; termination is graceful-first with a kill-and-confirm fallback. The 90-second default timeout remains above the canonical Node `exec` foreground/outer wait. This phase adds no lifecycle, profiles, ownership policy, Docker/bwrap/VM assumptions, workspace path, WebUI, slash command, fixed Node service, or generic compound-copy behavior.
