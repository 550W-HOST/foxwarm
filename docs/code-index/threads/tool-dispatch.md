...
- Main Management, remote Node execution, MCP external, bounded vector calls, file delivery, intermediate/final delivery, and presentation events are reverse-wired for the activated WorkerHost through one shared channel. Main Management version 2 retains its fixed 11-operation allowlist and carries no live Session, history, queue, patch, or callback. Unsupported operations still fail before effects rather than falling back to Main hydration.
