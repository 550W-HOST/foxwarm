...
3. Direct provider calls, unified `call_tool`, and ToolScript nested calls resolve one canonical `ResolvedTool` before permission or execution. Capability source (`builtin`, `node`, or `mcp`), execution target, and process/service owner remain separate. A unified invocation then authorizes the outer exact builtin `call_tool` identity before independently authorizing/executing its concrete target; MCP retains its authoritative Main service recheck for the concrete server/tool. Node-environment direct tools select the session's current node. Main-local production uses local facades, while Session-worker production borrows reverse Main Management, remote Node, MCP, vector, file, delivery, and presentation facades through the exact Worker source boundary.
...
- Unified wrappers require both the outer exact builtin `call_tool` capability and the concrete target's existing permission/service/structural guards. An outer allow never replaces a concrete allow, and an outer deny fences nested Node/MCP/handler effects.
