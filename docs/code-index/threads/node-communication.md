...
[2026-09-19] The current Master and first-party CLI now offer range 1-3. A negotiated-v3 authenticated CLI Node must additionally advertise `externalToolOwner:1` before Main dispatches a disjoint `{kind:'external',externalId,contextId}` `tool_call`. Missing capability, older negotiated v1/v2, malformed/mixed Session and external owner, unavailable Node, or unadvertised tool fails before effect; internal Session request envelopes and completion rights continue unchanged across v1/v2/v3. The first owner seam handles CLI file primitives and `get_default_cwd` only; browser, exec and other provider external paths remain separate work and are not model-facing at this intermediate point.
