...
Layered compaction plans from an exact cloned history snapshot. Commit requires deep equality of the complete consumed snapshot while allowing only an appended live suffix; it directly replaces selected history ranges with rendered CTX-BLOCK messages, copies requested exact raw messages with `preservedFromBlockId`, removes display-only consumed rows and old pure completion notices, retains the compatible appended suffix, and appends one current completion marker. Missing or ambiguous `seq`/block provenance protects the affected entry as a range barrier rather than consulting archive to recover content. Successful compact preserves `promptCacheKey`; `/clear` remains the rotation boundary.

Required archive appends are synchronous and fail closed for the affected Session mutation. Append helpers restore the in-memory semantic snapshot when archive or authoritative-state persistence fails, while Worker fencing/resync isolates the failure to that Session/turn. If authoritative JSON persistence fails after a new SQLite insert, only the exact rows reported as newly inserted by that same commit are removed before the error returns; pre-existing identical replay rows are never rollback candidates. Archive message/block same-key replay is idempotent only when the stored row is byte-semantically identical; conflicting content fails instead of overwriting immutable history.
