...
- Prior compact-completed notices are transparent candidate noise: they neither enter summaries nor split legal ranges. Other protected lifecycle items, preserved raw items, missing/duplicate/nonmonotonic active sequence structure, and non-candidate blocks are hard range barriers. Display-only messages are transparent and excluded from quota denominators.
