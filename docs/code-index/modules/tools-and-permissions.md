...
- [src-mcp-inbound-node-service](../units/src-mcp-inbound-node-service.md) — verified Main-local external owner seam and paired authenticated CLI Node file-tool dispatch; not yet registered as an inbound MCP wrapper.
...
- `checkToolPermission` and `checkPathAccess` — generic policy entry plus current isolation checks.
...
- The separate instance-level ordered policy applies to every Session after canonical target resolution. Its allow result never bypasses the current exact isolated rules or structural guards; its deny result may further restrict isolated or non-isolated callers.
...
[2026-09-03] `search_tools` returns only the standard `{ output: string }` envelope. The text begins with actual emitted/total-match counts and retains bounded warning text. Safe identities use compact canonical-ID declarations; unsafe identities use copyable explicit `call_tool` source/server-or-node/name descriptors with exact JSON quoting, and are omitted with a warning when structured identity is unavailable. Its default result limit is 5. Runtime normalizes finite values with truncation and a 1-200 clamp; omitted or non-finite values use 5, while the model schema is an integer bounded to 1-200. Schema detail remains capped at the first 10 selected results, and `includeSchema=false` emits explicit schema-omitted summaries. The complete formatter output, including headers, omission notices, and warnings, is capped at 32,000 characters below the outer tool-output guard. Metadata controls and line-direction characters are escaped. Recognized unsupported structural schema constraints widen to `unknown` or `Record<string, unknown>`. Required-only object keys retain other-key typing, open-object unknown typing, or closed-object impossibility, with bounded overflow marked explicitly; no claim is made that every future JSON Schema keyword is understood.
...
[2026-08-01, updated 2026-08-20] Keep the default model-facing namespace compact by grouping closely related operations under singular resource tools: `session` owns status/list/update-display-name, `skill` owns list/load, and `node` owns list/select/create/ensure/inspect/destroy. Do not add parallel `sandbox` or `node_resource` tools; provider-neutral lifecycle authority is canonical in [D-dispatch-generic-node-providers](../threads/tool-dispatch.md#d-dispatch-generic-node-providers). The display-name action is exactly `update-display-name`; the earlier `rename` action and removed standalone tool name are not retained as aliases. Keep action-specific arguments flat in the shared schema. A successful display-name update reports both the previous and resulting values, using `unset` for no display name and an explicit unchanged result for no-op calls. `start_toolscript_run` is the narrow exception to removed-name handling: it remains hidden and callable only for documented user-ToolScript compatibility, while current guidance uses `run_script({ mode: "background" })`. MCP configuration/listing is discoverable through `search_tools`/`call_tool` rather than injected by default.
