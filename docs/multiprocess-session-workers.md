...
- **Recall** — cross-session exact/vector recall and hidden archive range/block readers are closed through Main.
...
- final delivery, waits, ToolScript persistence, archive/journal writes, and background exec completion have one supported path — **met** (verified in the production test environment, including the `afterSend:"wait"` chain).
