...
| Normal ingress | `MessageRouter` routes ordinary busy/idle channel/WebUI input through the closed ensure/spawn/append/run ingress operation (`submitEnsuringWorker`), and all Main-side enqueue producers share the same durable boundary. Exact fenced stop and canonical retry use closed exact-owner operations; retry creates no mailbox item or second queue and rejects active overlap. Managed sessions and dequeue remain unavailable. |
...
Implemented: the version-3 main-management facade has a fixed 19-operation allowlist. In addition to child/catalog/message operations, it closes cross-session `recall` and hidden archive readers, Main-owned `create_agent`/`create_session`, and node bootstrap/pair list/approval. Reverse handlers fence exact generation/incarnation before effects and cap args; recall preserves scope/isolation and exact archive/vector source reload, while creation derives inherited settings from a detached source authority. Source conversion remains fenced. Real-child tests exercise every new operation, stale-source rejection, and direct/unified/ToolScript parity.
...
Implemented per operation: **stop** — control `stop` on a fenced session routes to the worker runtime `interrupt` through the exact supervisor fence. **retry** — slash/WebUI retry ensures the exact Worker and invokes the canonical `parts:null` retry turn with serialized source/final-delivery metadata; it has no mailbox row or duplicate history algorithm and rejects active overlap definitely. A lost response after invocation reports `SESSION_WORKER_RETRY_OUTCOME_UNKNOWN` and instructs history inspection because commit/delivery may already have happened; it never falls back or auto-retries. **dequeue** remains unavailable. **delete** — sessionManager delete entry points route fenced sessions through interrupt, graceful stop/handback, durable fence/mailbox deletion, then ordinary authority/catalog/archive deletion. **fork/archive/displayName** retain their closed detached/catalog-only paths.
...
- **Cross-session control/settings** — other-session dequeue and non-displayName settings remain unavailable; exact current retry is closed.
