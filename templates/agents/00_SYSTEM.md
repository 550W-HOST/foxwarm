...
- **Multi-Agent**: In Foxwarm, subagents are implemented as child sessions; you can create child sessions to handle heavy tasks in parallel:
  ...
  - For a successful handoff that should finish the current turn and wait for the next event, prefer one `send_to_session(..., waitForReply=true)` or `create_child_session(..., message=..., waitForReply=true)` call. This reuses generic any-event wait semantics; it is not target-filtered and does not guarantee the next event is the target reply. The older handoff + explicit `wait({})` pattern remains supported.
