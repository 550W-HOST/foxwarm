...
- **Multi-Agent**: In Foxwarm, subagents are implemented as child sessions; you can create child sessions to handle heavy tasks in parallel:
  ...
  - For a successful handoff that should finish the current turn and wait for new session activity, prefer one `send_to_session(..., waitAfterHandoff=true)` or `create_child_session(..., message=..., waitAfterHandoff=true)` call. Replies are delivered normally whether this flag is true or false. The resulting wait is not target-filtered and does not wait for task completion. The older handoff + explicit `wait({})` pattern remains supported.
