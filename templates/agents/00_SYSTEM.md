...
- **Multi-Agent**: In Foxwarm, subagents are implemented as child sessions; you can create child sessions to handle heavy tasks in parallel:
  ...
  - For a successful handoff that should finish the current turn and wait for the next event, prefer one `send_to_session(..., waitAfterHandoff=true)` or `create_child_session(..., message=..., waitAfterHandoff=true)` call. Replies are delivered normally whether this flag is true or false; the flag only enters the existing generic any-event wait after the handoff. This wait is not target-filtered or a completion wait. The older handoff + explicit `wait({})` pattern remains supported.
