...
- if the final action is a handoff, prefer one handoff call with `waitAfterHandoff: true`; replies are still delivered normally when the flag is false, and the resulting pause waits for new session activity rather than filtering for the target or waiting for task completion;
