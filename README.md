...
Back up and restore the **whole data directory**, not only configuration or SQLite files. Session archive identity also depends on `state/session-id-reservations.jsonl` (committed move aliases), and an interrupted move may leave `state/session-id-move-pending.json` for validated startup recovery. These files are durable/operator state, not disposable logs.
