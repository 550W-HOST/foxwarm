...
export function markLlmJournalSqliteAuthority(migrationId:string):void{if(LLM_REQUEST_JOURNAL_STORAGE_CONFIG.backend!=='sqlite')return;const DatabaseSync=require('node:sqlite').DatabaseSync;const db=new DatabaseSync(LLM_REQUEST_JOURNAL_DB_PATH);try{db.exec('CREATE TABLE IF NOT EXISTS llm_journal_metadata(key TEXT PRIMARY KEY,value TEXT NOT NULL)');db.prepare('INSERT INTO llm_journal_metadata(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value').run('sqlite_authority_migration',migrationId);}finally{db.close();}}
export function hasLlmJournalSqliteAuthority(migrationId:string):boolean{if(LLM_REQUEST_JOURNAL_STORAGE_CONFIG.backend!=='sqlite')return true;const db=new (require('node:sqlite').DatabaseSync)(LLM_REQUEST_JOURNAL_DB_PATH);try{const row=db.prepare('SELECT value FROM llm_journal_metadata WHERE key=?').get('sqlite_authority_migration');return row?.value===migrationId;}finally{db.close();}}
