...
export function buildModelsConfigFromSetupForm(body: any, existingConfig: any = {}): { default: string; providers?: Record<string, ProviderConfigEntry>; models?: Record<string, ProviderConfigEntry> } & Record<string, any> {
  ...
  for (const [index, draft] of drafts.entries()) {
    ...
    if (isVirtual) {
      ...
      for (const field of ['models', 'model', 'baseUrl', 'apiKey', 'requestCompression', 'extraFields', 'extraHeaders', 'webSearch', 'contextLimit', 'effort', 'historyReasoningField', 'asyncCompact', 'disallowEmptyResponse'] as const) {
