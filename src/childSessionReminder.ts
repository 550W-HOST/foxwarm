...
export function buildChildCompletionInstruction(parentSessionId: string): string {
  return `When you finish, explicitly call send_to_session({sessionId: \`${parentSessionId}\`, message: "..."}). If no parent reply is needed, end your final message with \`${NO_ACTION_MARKER}\`.`;
}

export function buildChildReminder(parentSessionId: string): string {
  return `message ended without send_to_session call. If you need to report back to the parent session, call send_to_session({sessionId: \`${parentSessionId}\`, message: "..."}). If no parent reply is needed, end your final message with \`${NO_ACTION_MARKER}\`.`;
}