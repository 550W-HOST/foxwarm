...
export function buildInvalidExecCwdMessage(resolved: ResolvedExecCwd, reason: string, nodeId?: string): string {
  ...
  return `Cannot start exec${formatNode(nodeId)}: working directory is invalid (${reason}). Source: ${resolved.source}.${rawPart} Resolved cwd: \`${resolved.cwd}\`. This is usually a cwd problem, not a missing \`/bin/sh\`; Node.js can report invalid cwd as \`spawn /bin/sh ENOENT\`.`;
}
