...
test('bounded cache owns exact watches, focus paths, pages, latest generations, and fixed coalescing', () => {
  ...
  assert.match(files.client, /source\.onopen = \(\) => \{ controller\.delay = 1000; requestSessionListStreamOpenResync\(schedulerRef\.current\) \}/)
...
test('Sidebar and collapsed rail consume server order without client tie sorting', () => {
  ...
  assert.match(files.list, /childTotal === null \? 'Children'/)
...
test('Architecture owns its bounded summary and forest instead of an all-session prop', () => {
  ...
  assert.match(files.architecture, /architectureSubscriptionIds\.length \?.*:\s*\[\[\]\]/s)
  assert.match(files.architecture, /source\.onopen = \(\) => requestSessionListStreamOpenResync\(scheduler\)/)
