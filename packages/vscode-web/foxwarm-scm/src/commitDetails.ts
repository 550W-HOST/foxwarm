...
function handleCommitMessage(message: unknown, details: CommitDetails, openEditor?: () => void): void {
  if (!message || typeof message !== 'object') return;
  const typed = message as { type?: unknown; index?: unknown };
  if (typed.type === 'openEditor' && openEditor) { openEditor(); return; }
...
export class CommitDetailsViewProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    view.webview.onDidReceiveMessage((message) => {
      ...
      handleCommitMessage(message, this.details, () => { void openCommitDetailsEditor(this.details!); });
    });
    view.onDidDispose(() => {
