...
## Foxwarm sidebar and WebUI editors
...
- Closing or reloading the whole VS Code browser page only detaches its WebSocket clients. The terminal extension activates after Code startup, lists existing backend terminals on the same node whose cwd is inside the current workspace, and recreates attach-mode terminal views without POSTing new PTYs. It registers every restored view hidden, then reveals the first one after a short delay so Code's Terminal panel is populated without racing the default profile into creating an extra PTY.
