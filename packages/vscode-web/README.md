...
`foxwarm-webui` contributes a dedicated **Foxwarm** Activity Bar container. Its WebviewView contains a thin outer bridge and an iframe loading the normal WebUI in the strict `foxwarmEmbed=sidebar` mode. That mode renders the session list, search/view controls, Agents entry, settings, and the existing New agent/New session dialogs, but not the WebUI workbench shell. The narrow settings menu is end-aligned so it remains inside the view. Session, Agents, and Setup selections send only fixed versioned messages checked against the exact iframe source plus a random per-view nonce.
